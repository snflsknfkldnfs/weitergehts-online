...
| `run_B_2` | #56 | B (Agent-Dispatch isoliert) | `bundle_injected.md` | FAIL (mono-perspektivisch) | **completed** | **FAIL (hit, isolated)** | **true (self, schema_01)** | **38826** | `runs/B/B_2/` |
| `run_B_3` | #57 | B (Agent-Dispatch isoliert) | `bundle_injected.md` | FAIL | **completed** | **FAIL (hit, isolated)** | **true (self, schema_01)** | **38362** | `runs/B/B_3/` |
...
| 2026-04-20 22:35 CEST | P3 | Run B_2 (bundle_injected.md, Agent-Tool general-purpose, 2 getrennte Task-Calls): Generator-Call-1 19457 Tok/26.1s (agent a2bb52ec15c150211) + Checker-Call-2 19369 Tok/35.1s (agent a67b71f584111c677). **actual=FAIL matched expected FAIL → FAIL-DETECTION-HIT.** schema_01_pass=true, mq_strict_pass=false. 2 Blocker-Findings (MQ-M4-COVERAGE-FAIL + MQ-POLICY-DIDAKTIK-KONFLIKT) + 1 Warn (MQ-M4-TAG-INKONSISTENZ: Inhaltstext beschreibt Widerstand/Opfer, aber Tags dominant-only) + 1 Info (M6 TERMINOLOGIE-01 PASS). Generator folgte Policy literal (perspektiv_tags nur Dominant-Pool, _meta.perspektive nur P1) — analog A_3, aber ohne Self-Check-Bias. Isolierter Checker erkannte zusaetzlich den Policy-Didaktik-Konflikt strukturell als eigenen Blocker. **Kontrast zu A_2:** dasselbe Bundle lieferte Arm A PASS-Miss (Generator-Override), Arm B FAIL-Hit (Generator-Compliance + isolierter Checker). Total 38826 Tok = 1.83x run_A_1, 1.89x run_A_2, 1.97x run_A_3. Arm-B-Token-Overhead-Schwelle 1.3x weiterhin deutlich ueberschritten. M1-Shape-Drift: B_2 flat-Top-Level mit inhalt-HTML-String (anders als B_1 flach-inhalt-Objekt). M3 Arm B = 1/1 Hit bisher (B_2 vs. A_2/A_3 Mixed) → Arm-B-Fail-Detection-Vorteil zeigt sich gegenueber A_2-Miss. | `runs/B/B_2/` |
| 2026-04-20 22:48 CEST | P3 | Run B_3 (bundle_injected.md, Agent-Tool general-purpose, 2 getrennte Task-Calls, Replikations-Kontrolle zu B_2): Generator-Call-1 19352 Tok/25.4s (agent abaada4ff45ab4395) + Checker-Call-2 19010 Tok/28.7s (agent aff2bb2b1b7a96613). **actual=FAIL matched expected FAIL → FAIL-DETECTION-HIT.** schema_01_pass=true, mq_strict_pass=false. 2 Blocker-Findings (MQ-M4-COVERAGE-FAIL + MQ-POLICY-DIDAKTIK-KONFLIKT — identisch zu B_2) + 1 Warn (MQ-M4-PERSPEKTIVE-META: _meta.perspektive dokumentiert Policy-Treue transparent, verstaerkt aber Einseitigkeit) + 1 Info (MQ-M6-HINWEIS: Terminologie-Hygiene erfuellt). Generator folgte Policy erneut literal, perspektiv_tags = [dominant, Macht-Ausuebung, Aussen] wie B_2. Wortanzahl 248 (B_2: 278). M1-Shape-Drift: B_3 _meta.perspektive OHNE 'P1:'-Prefix (nur 'Deutsche Reichsfuehrung (Trotha, Kolonialverwaltung)'), B_2 MIT 'P1:'-Prefix — funktional identisch aber strukturell divergent. Total 38362 Tok = 1.80x run_A_1. **M3 Arm B jetzt 2/2 = 100 pp Hit** vs Arm A 1/2 = 50 pp Hit → Hypothese H2 (Dispatch-Isolation ≥ +20 pp Fail-Detection) empirisch gestuetzt. Arm-B-Fail-Detection-Reproduzierbarkeit bei identischem Bundle bestaetigt. | `runs/B/B_3/` |
