...
10. NEU Phase 0.2.Z "Zitat-Baustein-Kuratierung" (R0.7): Optionale Sub-Phase parallel zu 0.2.M, aktiv nur wenn bpb (oder andere CC-BY-NC-ND-Quelle) fuer ein Game gewaehlt wird. Sub-Agent `bpb_zitat_kurator` extrahiert wortgetreue Kurzzitate (1-5 Saetze) in `zitat_katalog_game.json`. Tools: markdownify + WebFetch. Invarianten I1-I6 in Befund §7.2.
...
12. NEU Sub-Agenten-Invariante "Lizenz-Pre-Check" (R0.7): Fuer alle Nicht-Wikipedia-Quellen gilt vor Ingest ein Lizenz-Check. Quellen mit ND-Klausel duerfen nicht in die Volltext-Ingest-Pipeline, sondern nur in die Zitat-Pipeline (Phase 0.2.Z). Pflicht-Gate-Hook analog zu Titel-Validierung aus R0.6.
13. NEU medien_katalog_game.json Schema-Erweiterung (R0.7 §12.2): Felder `bpb_verifiziert`, `bpb_discovery_url`, `bpb_bildunterschrift_zitat`, `bpb_didaktische_einordnung`. Sub-Agent 0.2.M erweitert um parallele bpb-Abfrage, Match ueber Bundesarchiv-ID / Bildname. bpb-Kuratierung als didaktischer Qualitaetsstempel, reduziert Sichter-Workload in Runde 5.
14. NEU Artefakt `primaerquellen_katalog_game.json` (R0.7 §12.3-§12.4): Paralleler Katalog-Typ zu medien_katalog_game.json und zitat_katalog_game.json. Enthaelt PD-verifizierte Primaerquellen (Reden, Erlasse, Tagebuecher, Gedichte, amtliche Werke) inklusive Original-Archiv-URL. Darf in Phase 0.1 Volltext-Ingest-Pipeline, weil PD = keine Bearbeitungs-Beschraenkung. Discovery erfolgt ueber bpb-Artikel, Zitat erfolgt aus Original-Archiv (Wikisource, Deutsches Textarchiv, Bundesarchiv).
...
16. NEU Sub-Agent `bpb_primaerquellen_extraktor` (R0.7 §12.4): zweiter Sub-Agent in Phase 0.2.Z neben `bpb_zitat_kurator`. Workflow: markdownify → Regex/Heuristik auf Primaerquellen-Zitate → Autor-Todesjahr via `mcp__wikipedia__get_summary` → PD-Regel `todesjahr + 70 < aktuelles_jahr` → Original-Archiv-Suche via WebSearch auf Wikisource/DTA/Bundesarchiv → Aufnahme in primaerquellen_katalog_game.json. Invarianten PQI1-PQI6. Amtliche Werke (§5 UrhG) auto-PD ohne Todesjahr-Check.
**Offene Folgetickets:**
