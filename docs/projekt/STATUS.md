...
**Predecessor-Bridge-Pairs:** p4 `614a35ad` (Trace-RE Run-4 closed 2026-04-28) + p5 `04c32c03` (v0.6-Architektur-Spec closed 2026-04-30).
**Spec-Anker:** `escape-game-generator/docs/projekt/PLUGIN_v0_6_ARCHITEKTUR_SPEC.md` (395 Z., 10 Sektionen).
**Zielsystem-Anker (NEU):** `escape-game-generator/docs/projekt/ZIELSYSTEM_ANKER.md` (Pflicht-Referenz fuer alle Bridge-Pairs + Worker-Sessions).

### Sub-Tracks

| Sub-Track | Inhalt | Status | Verantwortlich |
|---|---|---|---|
| **T1 Run-5 Pristine-Test** | Run-5 mit Plugin v0.5.2 + Stream-JSON-Format-Pflicht (Game-Thema NICHT Erste-Weltkrieg) | OFFEN — User-Aktion Code-Mode | User Code-Mode-Session |
| **T2 Quellen-Layer-Refactor-Spec** | Bridge-Pair p7 fuer v0.6-Spec-Erweiterung um R3.4-NEU Quellen-Layer (LeMO + DDB + bpb + WLO/MUNDO) | OFFEN — Cowork-Aktion | Cowork Bridge-Pair p7 |
| **T3 v0.6.0 Implementation** | R1+R3-Layer-Implementation post-T1+T2-Closure | DEFERRED — Anti-Drift-#1 (Run-5-Empirie pending) | Bridge-Pair p8 (deferred) |
...
T1 (Run-5) -------+
                  |
                  +--> T3 (v0.6.0 Implementation, deferred)
                  |
T2 (p7-Spec) -----+
```

T1 + T2 parallel, T3 post-T1+T2-Closure.

### Anti-Drift-Regel-Carry-Over (5 Regeln, NEU #5)

| # | Regel | Quelle |
|---|---|---|
| 1 | Kein Plugin-Patch-Sprint ohne Run-5-Empirie | p4 |
| 2 | Keine R2 Pattern-Library ohne User-Paedagogen-Input | p4 |
| 3 | Keine Validatoren ohne strukturell bezifferte Substanz | p4 |
| 4 | Keine WARN-Hook-Inflation (nur BLOCK-Hooks) | p4 |
| **5** | **Keine Spec-Empfehlung / Sprint-Akzept / Bridge-Decision ohne explizite Cross-Reference zu mindestens einem Zielsystem-Anker-Item.** Worker + Advisor verifizieren jede substantielle Aussage gegen Zielsystem-Anker + markieren `zielsystem_cross_refs: [array]` im Handover. | NEU 2026-04-30 |

### Out-of-Bridge Closure-Tasks aus p4+p5 (Status)

- A.1 WORKFLOW_v4-Stale-Marker-Commit: DONE (weitergehts-online commit 592c402)
- A.2 v0.5.1+Phase-D-Validator-Run gegen Run-4: DONE (75% HIGH-Catch, Forecast 50-70% UEBERTROFFEN)
- A.3 Memory-Update post-Validator-Run: DONE
- p5 C.1 v0.5.2-Patch-Implementation: DONE (commit 066d919, tag v0.5.2)
- p5 C.2 Run-5-Setup-Vorbereitung: OFFEN (= T1 oben)
- p5 C.3 Memory-Update project_v06_spec.md: DONE

### Naechster Schritt

**T1 Run-5 + T2 p7-Bridge parallel starten.** Pre-T2-Pflicht: ZIELSYSTEM_ANKER.md schreiben (#2 von 5 PM-Vorbereitungs-Schritten).



**Letzte Aktualisierung:** 2026-04-27 (**Run-4 Pilot-Cancel + 4 Hotfix-Wellen + 7 BEFUNDe + Track 2 v0.5.1-Phase-A KOMPLETT (13/13 HIGH-Items DONE)** — Phase A Implementation **DONE** im Generator-Repo (Commits `317626e` + `6a633b1`): **6 NEUE Tools** (check_hefteintrag_struktur + check_synthese_aufgabe + check_rahmen_completeness + check_img_id_pattern + check_doppelfragen + check_ueberleitung_qualitaet, ~1500 LOC Python) + **6 NEUE Hooks** (4 PostToolUse + 1 PreToolUse + 1 weitere) + **24 Subagent-Frontmatter-Patches** (F-PB-72 state-advance-policy Pre-Run-State-Pflicht in 24/24 Subagents) + **11 Vertrag-Patches in 7 Agent-Files** (F-PB-50/85/87/88/89). **v0.5.1-Backlog erweitert auf 39 Items** post User-Pädagogen-Befund 2+3 (Doppelfragen-Verbot F-PB-88 + Überleitungs-Qualität F-PB-89). **Plugin-Komponenten-Inventar v0.5.1-Phase-A:** Tools 23→29 (+6) | Hooks 17→23 (+6) | 24/24 Subagent-Frontmatter mit F-PB-72 | 11 Vertrag-Patches in Agent-Files. **Run-4-Empirie auf NEUE Validatoren:** check_hefteintrag_struktur M1 PASS post-Cowork-Hotfix + M2 2 Errors / check_synthese_aufgabe M2+M4 Errors / check_rahmen_completeness mappe-2/rahmen/ MISSING / check_img_id_pattern 0 Errors / check_doppelfragen 1+6 / check_ueberleitung_qualitaet 0 Floskel post-Beispiel-Patch. **Aufwand-Ist Phase A:** ~2.5h Wall-Clock (Plan: 19-26 PT — Speedup ~85% durch Self-Edit + Python-Batch + Subagent-Pattern für Frontmatter-Patches). **8 BEFUNDe konsolidiert:** AUDIT (236) + TIEFEN_EVAL (314) + GENERIERUNGS_TRACE (305) + LIVE_CONTENT_INVENTAR (365) + BACKLOG_KONSOLIDIERT (368) + CONTENT_QUALITAET (253) + FRAGEBOGEN_QUALITAET (241) + DIDAKTISCHE_STRUKTUR (262 inkl. §8 Doppelfragen+Überleitungen) = 2344 Z. strukturierte Empirie-Doku. **Total Hotfix-Patches Run-4:** 852 (3 Cowork-Wellen + Restdrifts + Lück→Lüttich + M1-Hefteintrag-Optimierung + 2 Beispiel-Überleitungen). **Methodik-Lessons etabliert:** (1) User-Pädagogen-Audit als 4. Pflicht-Audit-Stufe (neben Sandbox-Validatoren + Subagent-Reviews + Plugin-Self-Diagnose). (2) Plugin-vs-Engine Schema-Drift = häufigste CRITICAL-Bug-Klasse (F-PB-77/78/79/80/82). (3) Ende-zu-Ende Browser-Render-Test (headless Playwright) als Pflicht-v0.5.1-Phase-B-Item. **Naechste Schritte:** **Plugin-Validator-Run user-side** (`claude plugin validate`) → **Phase B starten** (Schema-Drift-Cluster F-PB-65/66/75/76 + 6 weitere MED, ~6-8 PT, 2-3 Wochen) → Run-5-Pristine-Test mit neuem Game → v0.5.1-Tag. Vorheriger Eintrag — **Run-4 Pilot-Cancel + 4 Hotfix-Wellen + 7 BEFUNDe + Pfad C Track 1 DONE + Track 2 v0.5.1-Phase-A IMPLEMENTATION GESTARTET** — User-Decision: **Pilot wird ausfallen** (User-Pädagogen-Expertise reicht für Eval-Ableitung des erwartbaren Ablaufs ohne SuS-Empirie). Track 1 Cowork-Sprint **DONE**: **Hotfix-Welle 1** F-PB-77/78/79 (Bilder-URL + Lückentext-Marker + Umlaut-Bulk-Replace 807) Commit `7a686ff`. **Hotfix-Welle 2** F-PB-80 reihenfolge.elemente→optionen Commit `5ac7d26`. **Hotfix-Welle 3** F-PB-82 (Lück→Lüttich, 4x) + M1-Hefteintrag-Optimierung (8 verbindungen + 2 voraussetzungen + ordnungsmuster=multi-kausal-verschränkt + transfer.frage + 4 SCPL.zwischen_komplikationen + a-1-7 als echte Synthese-Aufgabe alle 6 Materialien + Punkte 20→25) + 5 Restdrifts (plotzlich/staerker/Pruefe/Stadtbuerger/Taeter) Commit `14d9f85`. **Total Hotfix-Patches:** 4 + 14 + 8 + 4 + 1 + 9 = 31 strukturelle Patches im data.json + 821 vorherige Replacements = 852 Patches gesamt. **7 BEFUNDe konsolidiert:** BEFUND_RUN4_AUDIT (236 Z.) + TIEFEN_EVAL (314 Z.) + GENERIERUNGS_TRACE (305 Z.) + LIVE_CONTENT_INVENTAR (365 Z.) + BACKLOG_KONSOLIDIERT (368 Z.) + CONTENT_QUALITAET (253 Z.) + FRAGEBOGEN_QUALITAET (241 Z.) + DIDAKTISCHE_STRUKTUR (173 Z.) = 2255 Z. strukturierte Empirie-Doku. **v0.5.1-Backlog-Stand:** 33 → 37 Items (4 NEU aus User-Pädagogen-Befund: F-PB-84/85/86/87 didaktische Strukturqualität). Phase A wächst auf 11 HIGH-Items. Aufwand-Schätzung 17-24 PT, ~3-4 Wochen bis v0.5.1-Tag. **Track 2 v0.5.1-Phase-A-Implementation gestartet** im Generator-Repo (8 HIGH-Items: F-PB-50/72/63/52/53/84/85/87, ~7-10 PT). **Methodik-Lesson:** User-Pädagogen-Audit als Pflicht-Audit-Stufe etabliert (5 vorherige BEFUNDe + CLI-Self-Eval haben M1-Hefteintrag-Strukturmängel nicht detektiert). **Naechste Schritte:** v0.5.1-Phase-A-Implementation komplett (Generator-Repo) → v0.5.1-Phase-B Schema-Drift-Cluster → Run-5 Pristine-Test mit neuem Game → v0.5.1-Tag → Pilot-Reanimation falls neue Empirie gewünscht. Vorheriger Eintrag — **Run-4 Live + F-PB-68 FIX + 5 BEFUNDe konsolidiert + Pfad C Pilot-Vorbereitung GESTARTET** — Pfad C aus 4-Optionen-Plan: Parallel Pilot-Vorbereitung + v0.5.1-Phase A. **Stufe A DONE:** F-PB-68 lehrkraft.html JS-Renderer fuer mappenabschluss.lehrkraft*-Felder (198 LOC neu, 3 Renderer-Funktionen + Skip-Liste + Spezial-Banner POLICY_TRIGGER_SICHTBARKEIT). Cowork-Commit `a5c998c`. Live-Verifikation post-30s GitHub-Pages-Build: 19860 B, 2x F-PB-68 + 2x renderMappenabschlussLehrkraft + 1x POLICY_TRIGGER_SICHTBARKEIT Marker. Lehrkraft-Frontend zeigt jetzt 3 Mappen-Bloecke (M1 Didaktischer Rahmen + M3 Trigger-Sichtbarkeit lehrkraft-only + M4 Lehrkraft-Sektion) statt statischer Platzhalter. **PILOT-BLOCKER BESEITIGT.** **Stufe B DONE:** PLUGIN_v0_5_1_HARDENING_SPEC.md im Generator-Repo (10 Sektionen + 27-Item-Backlog strukturiert + 3 Implementations-Stufen + Run-5-Akzeptanzkriterien + Migrations-Hinweise + Commit-Plan). **Stufe C DONE:** PILOT_WALK_THROUGH_M2_M3_M4.md (Lehrer-Briefing fuer Pilot-Einsatz: M2 Princip-Multiperspektivitaet + M3 Beutelsbach Versailles vs. Clark + M3 Augusterlebnis-Idealisierungs-Verbot + M3 Versailles-NSDAP-Sprengkraft V13 + M4 Schlieffen-Plan-Mythos-Korrektur Zuber 2002+ + Game-Abschluss-Reflexion + Pilot-Eval-Pflicht). **5 BEFUNDe konsolidiert (vorab):** BEFUND_RUN4_AUDIT (236 Z.) + BEFUND_RUN4_TIEFEN_EVAL (314 Z.) + BEFUND_RUN4_GENERIERUNGS_TRACE (305 Z.) + BEFUND_RUN4_LIVE_CONTENT_INVENTAR (365 Z.) + BEFUND_RUN4_BACKLOG_KONSOLIDIERT (368 Z.) = 1588 Z. strukturierte Empirie-Doku. v0.5.1-Backlog konsolidiert auf 27 Items (6 HIGH + 10 MED + 11 LOW), ~14-20 PT, ~3-4 Wochen bis v0.5.1-Tag. CLI-Self-Eval-Verifikation: 7/8 Anti-Patterns empirisch belegt + 1 Mis-Diagnose korrigiert (A6 Verifier-Source-Confusion war keine — data.json.pre-hotfix.bak hatte tatsaechlich 3 Schemata gemischt). **Naechste Schritte:** Pilot-Termin mit Lehrkraft koordinieren (Lehrkraft-Login Walk-Through + Pilot in 7c-GPG-Slot) PARALLEL zu v0.5.1-Phase-A-Implementation (F-PB-50 Pfad-Schema + F-PB-72 state-advance-policy + F-PB-63 rahmen-Pflicht + F-PB-52 SK5-Wortzahl + F-PB-53 img_id-Strict, ~5-7 PT, 1-2 Wochen). Vorheriger Eintrag — **Run-4 v0.5.0 KOMPLETT (Phase 0-3.5) + Hotfix DONE + Plugin-Self-Diagnose CLI-Capture** — Run-4-Game `gpg-erster-weltkrieg-ursachen-run4-v050` autonom durch Code-Mode-CLI generiert (~5h Wall-Clock Phase 0.1-2.2c + Phase 3.0-3.4 Assembly + 3.X Browser-Smoke). 4 Mappen / 22 Materialien / 28 Aufgaben. Game-Abschluss-Code: PULVERFASS — AUSLOESER — AUGUSTBEGEISTERUNG — STELLUNGSKRIEG. **Audit-Pre-Phase:** 6 Subagent-Reviews (Rev-A/B/C/D/G/H) + 7 Plugin-Validatoren + Drei-Wege-Vergleich Goldstandard/Run-3/Run-4 = `BEFUND_RUN4_AUDIT.md` (236 Z., 9 Sektionen, 3 HIGH + 4 MED + 8 LOW Findings + v0.5.1-Backlog 13 Items). **Cowork-Hotfix DONE:** (1) Bildpfad: 12 Bilder von Wikimedia Commons lokal gespiegelt (assets/img/{game-id}/, ~10.6 MB), 12 Pfade auf kanonisches Schema B vereinheitlicht (../../assets/img/{game-id}/img-mN-0n.{ext}), 9 Schema-A + 3 Schema-C-Wikimedia-Hotlinks eliminiert. (2) HIGH-Patches: img_id Bulk-Rename img-MN-n → img-mN-0n (288 Replacements in 20 TARGET-Files + 24 in 19 Mappen-Files), knoteen_id-Tippfehler-Fix mappe-3 mat-3-2. (3) LOW-Patches: a4-7 _meta.bloom "Analysieren" → "L4" + bloom_label, a4-7 scaffolding Quote-Bug ASCII U+0022 → German U+201C. **Validator-Ergebnis post-Hotfix:** artefakt_inventar 14 errors → 1 (UNKNOWN_FIELD f_pb_* → v0.5.1-Backlog). **Browser-Smoke 13/13 HTTP 200** (HTML + JSON + 7 Bildpfade-Stichprobe + CSS + JS). **Phase 3.5 git commit DONE:** Cowork-Bundle 157 Files / 28126 insertions, Commit `ba808b2` weitergehts-online/main, Push `59a7985..ba808b2`. **Plugin-Self-Diagnose CLI-Capture:** CLI-Generator-Session-Export (1115 Z.) hat 5 Plugin-v0.5.1-Issues selbst-diagnostiziert. CLI-1 (Bildpfad-3-Schemata, dedupt mit F-PB-50-NEW) + CLI-2 (M2 fehlt rahmen/-Verzeichnis, **NEU** F-PB-63-NEW) + CLI-3 (9 JSON unescaped typografische Anführungszeichen, post-write-Hook fehlt, **NEU** F-PB-64-NEW) + CLI-4 (Hefteintrag-Schlüssel-Drift tafelbild_knoten vs. knoten, Vertrag/Engine-Schema-Vereinheitlichung, **NEU** F-PB-65-NEW) + CLI-5 (lueckentext-Feldname-Drift → text_mit_luecken, **NEU** F-PB-66-NEW). v0.5.1-Backlog erweitert: 13 (Audit) + 4 (CLI) = **17 Items**. BEFUND_RUN4_AUDIT.md §10 Plugin-Self-Diagnose-Sektion ergänzt. **Naechste Schritte:** B2 Tiefen-Eval Run-4 (Drei-Wege-Vergleich Goldstandard/Run-3/Run-4 quantitativ + Plugin-v0.5.0-Hardening-Wirkungsanalyse + Hallu-Pipeline-Empirie + Architektur-Befunde S1-S4 Status + v0.5.1-Backlog REVISIT mit empirischer Pflicht/Optional-Klassifizierung + Phase-3.6-Deploy-Reifegrad-Urteil) → B3 Informed-Decision (v0.5.1-Backlog Implementation oder Phase 3.6 Deploy mit Lehrkraft-Pilot oder beide parallel). Vorheriger Eintrag — **Plugin v0.5.0 Phase C+D KOMPLETT — v0.5.0 Implementation TOTAL DONE** — 1 weiterer Commit Generator-Repo (939a527 Phase C+D F-PB-39+44+47+48+49). 5 Files / 863 Insertions / 1 NEW Tool. **Aufwand-Ist Phase C+D:** ~40-60 Min Wall-Clock (Plan 4 PT). **F-PB-Coverage v0.5.0 KOMPLETT:** 14 von 14 systemischen Findings + F-PB-04 strukturell adressiert. Alle Layer synchron: Schema (4 Phase-0-Schemas) + Validator (6 Tools) + Hook (5 Phase-0-Hooks) + AGENT (5 Files mit ~30 Patches) + VERTRAG (3 Files Pflicht-Q-Gates+Schema-Felder) + Tool (17 Tools total, 6 v0.5.0-spezifisch). **2 NEW Tools Phase C+D:** check_komposita_erstgebrauch.py (R7-Komposita-Erstgebrauch-Validator, 5 Erklaerungs-Methoden, Coverage-Schwellwert >=80%) + validate_skript_sk.py SK5-Wortzahl-Erweiterung (600-900W-Chunk-Korridor). **Run-3-Empirie neuer Tools:** Komposita-Coverage 24.2% FAIL + SK5-Wortzahl 0/6 Chunks im Korridor — strukturelle Plugin-Generator-Issues sichtbar gemacht (genau das war F-PB-44+47-Ziel). **8 Plugin-Commits seit v0.4.3:** 40fc423 Spec + 3eb7d7a A1 + 54c7e95 A2+A3 + 45a2502 A4+A5 + 3009ce2 B.1 + f70d69a B.2 + b305207 B.3 + 939a527 C+D. **Naechste Schritte:** T-v0.5.0-Smoke-Suite + Plugin-Version-Bump v0.4.3->v0.5.0 + Tag v0.5.0 (separater Stufe-1-Plan, in Vorbereitung) + Run-4-Regression. Vorheriger Eintrag — **Plugin v0.5.0 Phase B HIGH-Findings KOMPLETT** — 3 weitere Commits Generator-Repo (3009ce2 B.1 Phase-0.2-Schicht F-PB-37/38/41, f70d69a B.2 Phase-0.3-Schicht F-PB-36/40/42, b305207 B.3 Phase-0.1+Tools F-PB-43/45/46). 13 Files / 1396 Insertions / 3 Pushes auf main. **Aufwand-Ist Phase B:** ~2-3h Wall-Clock kumuliert. **F-PB-Coverage post-Phase-B:** 9 von 9 HIGH-Findings strukturell adressiert (AGENT-Generation-Layer + Validator-Layer + Hook-Layer komplett synchron). Plus 2 neue Tools: `validate_skript_sk.py` (SK1/4/5/7/14/18-Validator) + `check_math_consistency.py` (Header-vs-Body-Counts) — beide Self-Test PASS. Plus 1 neuer Hook `post-assemble-math-check` (Hook-Inventar 16 → 17). **Run-3-Empirie neuer Tools:** SK4 (kein §N-Markierung) + SK7 (keine Multikausal-Konnektoren) systematisch FAIL — neuer empirischer Befund, Tool macht Gap explizit (genau das war F-PB-43-Ziel). Math-Check auf artefakt_inventar PASS dank Cowork-Patch P8. **v0.5.0-Coverage:** Foundation (10.5 PT) + Phase B (11.5 PT) = 22 PT von 26 PT — F-PB-04 + 9 HIGH adressiert. **Verbleibend:** Phase C MED (F-PB-39/44/47/49) + Phase D LOW (F-PB-48) = 5 Findings, ~4 PT, separater Stufe-1-Plan in Folgesession. Vorheriger Eintrag — **Plugin v0.5.0 Foundation Phase A KOMPLETT** — 3 Commits Generator-Repo (3eb7d7a A1 Schema-Verzeichnis-Konsolidierung F-PB-04, 54c7e95 A2+A3 4 Phase-0-Schemas + 4 PostToolUse-Hooks, 45a2502 A4+A5 4 Validatoren + Doku + .gitignore-Hardening). 17 Files / 3425 Insertions / 3 Pushes auf main. **Aufwand-Ist:** ~70 Min Wall-Clock (Plan 10.5 PT ~5h — Subagent-Pattern + Self-Edit-Fallback Speedup ~50%+). **F-PB-Coverage post-Foundation:** Schema-Layer adressiert F-PB-37/38/39/40/41/42/45/46/48/49 (10 von 14); Hook-Layer 4 PostToolUse-Hooks aktiv via Wrapper; Validator-Layer Domain-Logic + Cross-Checks. **Run-3-Empirie gegen Foundation-Validatoren:** didaktik_rahmen rc=1 WARN, inhalts_briefing rc=2 FAIL (23 event_date), medien_katalog rc=2 FAIL (lizenz_summary + fehlende quellenkritik + aufnahme_datum-Coverage 6.2%), artefakt_inventar rc=1 WARN (Cowork-Patch v0.4.3 wirksam, 25 Materialien Pflicht-Felder erfuellt). Foundation-Hardening empirisch wirksam — Run-3-Defizite werden vom Plugin-Tooling jetzt automatisch detektiert. **Naechster Schritt:** Phase B HIGH-Findings (11.5 PT, F-PB-36 Drift-Pipeline + F-PB-43 SK1-SK18-Validator + F-PB-44 Komposita-Erstgebrauch + F-PB-47 SK5-Wortzahl + AGENT-Patches) als separater Stufe-1-Plan in Folgesession. Vorheriger Eintrag — **Phase 5 Forensik Run-3 DONE — Plugin v0.4.3 Hardening-Diagnose** — Stufe-1-Plan + 5-Stufen-Freigabe + Pfad-B sequenziell. Phase 5A Backlog-Cross-Check (14 Audit-Findings F2-F16 ohne Doppel-Tracking → neue F-PB-36 bis F-PB-49). Phase 5B Subagent-Forensik Variante B (~6 Min Wall-Clock, general-purpose Subagent, 14 Hypothesen verifiziert): **13 CONFIRMED + 0 REJECTED + 1 INKONKLUSIV** (F-PB-48 Schema-Naming). Severity 9 HIGH + 4 MED + 1 LOW. **4 strukturelle Architektur-Befunde:** S1 Plugin hat KEINE Schemas fuer Phase-0-Artefakte (Wurzel von 9/14 Findings); S2 Q-Gates sind LLM-Self-Checks ohne ausfuehrbare Validatoren (Ausnahme sprachniveau-gate.js); S3 alle 9 Hooks adressieren nur Phase 2/3, kein Phase-0-Hook; S4 VERTRAG_PHASE_0-3_SKRIPT.md ist die reale Datei, AGENT_ARTEFAKT hat keinen dedizierten Vertrag. **F-X6** (Cowork-Patches v0.4.3 nicht im Generator-Repo) ist KEINE Strict-Separation-Verletzung — Patches korrekt im TARGET-Repo, semantisch dasselbe wie F-PB-42. **X4** (Schema-Verzeichnis-Drift) ist bereits F-PB-04. **Phase 5C** Verifikation Akzeptanzkorridor PASS (BEFUND_FORENSIK_PHASE5_RUN3.md persistiert in docs/projekt/, ~280 Zeilen, alle Hardening-Pointer pro Komponente). **Run-3-Stand zum Zeitpunkt:** Phase 0.1+0.2+0.2.M+0.3 PASS, 12 Cowork-Patches (5 Stufe-1+2 + 7 post-Continuation), Phase 0.4 entsperrt aber zurueckgehalten. **Naechster Schritt:** v0.5.0-Hardening-Spec ableiten (separater Stufe-1-Plan), DANACH Phase 0.4 in CC. Aufwand-Ist Phase 5: ~10 Min Plan + 6 Min Subagent + 5 Min PM-Pflege = ~21 Min. Vorheriger Eintrag — **Plan-Konsolidierung UPGRADE_PLAN v3-12 v1.6 → v1.7 (Track-P.2-Closure)** — §23 NEU mit 6 Sub-Sektionen (~120 Z.): §23.1 Track-P.2-Closure-Status mit Phase-A+B-Commit-Tabelle + Plugin-Inventar (9 Hooks/7 Skills/16 Tools/18 Transitions/29 Frontmatter), §23.2 Track-C0-C10-Subsumption-Tabelle (alle Tracks C1-C9 + Teile C10 + Teile Track D abgedeckt durch P.1+P.2; verbleibend offen: §22.15 Tier-1/Tier-2 + agent-teams-Feature, beide post-Pilot), §23.3 Pilot-Roadmap (8-Schritte-Pfad: T-P.2-Empirie → Audit Phase B → Pilot-Setup → T-Full-Game-Pilot → 5-RA-Audit → Pflicht-Fix-Cycle → Lehrkraft-Review → Pilot-Freigabe-Entscheidung; **Total ~3-4 Wochen** revidiert von vorheriger Schaetzung ~10-12 Wochen durch P.1+P.2-Speedup), §23.4 v1.7 Delta=0 neue PI-Items (Aggregat 44 alle durch P.1+P.2 abgedeckt), §23.5 Roadmap-v2.3→v2.4-Update separat-pending, §23.6 Status-Marker v1.7. **Aufwand-Ist: ~25 Min.** Naechster Schritt: Pilot-Roadmap Schritt 1 (T-P.2-Empirie via Code-Mode-CLI) oder Schritt 2 (Audit Phase B). Final-Commit pending. Vorheriger Eintrag — **T-P.2-Smoke DONE + F-T-PA-01-Patch + BEFUND-Persistenz — Track P.2 Phase A+B EMPIRISCH VALIDIERT** — 8-Stufen-Smoke-Suite mit 14 Tests: Plugin-Validator PASS / 9 Tool-Self-Tests PASS / Mock-Workflow validate+render+runner FAIL+PASS-Pfade rc=2/0 korrekt / 9 Hooks active (4 Pre + 5 Post) / 7 Skills SKILL.md-Discovery / 18 Transitions 19 Phases 10 reale Plugin-Subagents (0 Phantom) / 29 Subagent-Frontmatter / Hook-Payload-Parsing pre-pi-phase-advance PASS / 8/8 SUB_AUFGABE Cross-Refs. **F-T-PA-01 (LOW) IM-CYCLE-GEFIXED:** Drift state-advance-policy SKILL.md `**Status:**` Markdown-Bold vs check_q_gate_log.py STATUS_RE `- Status:` List-Item. Patch: STATUS_RE-Regex erweitert auf 4 Konventionen. Re-Test PASS. **BEFUND_T_P2_SMOKE.md persistiert (~250 Z., 10 Sektionen):** Test-Stufen + Hook-Inventar (9) + Skill-Inventar (7) + Tool-Inventar (16) + Findings + Akzeptanzstatus 12/12 + Phase-C-Status + offene Punkte (4 LOW) + Akzeptanz-Empfehlung. **Akzeptanzkriterien Track P.2: 12/12 PASS.** Track P.2 Phase A+B substanziell + empirisch komplett. Pilot-Einsatz-Bereitschaft erreicht. **Wall-Clock Total Phase A+B+T-P.2-Smoke: ~6h** (Plan-Schaetzung ~3 Wochen → Speedup ~50x). Phase C (B.13+B.14, 4 PT) optional/deferrable. Aufwand-Ist T-P.2-Smoke: ~30 Min. Gen-Commit `a993bcb`. Vorheriger Eintrag — **Track P.2 Phase A + Phase B KOMPLETT (31/31 PT, 100%) — B.12 + B.16 DONE Bundle (Self-Edit, 2.5 PT, ~20 Min)** — **B.12 (1.5 PT, F-D2-03 + F-D1-02):** NEU skills/phase-prerequisites-check/SKILL.md (~7 kB). Auto-Trigger bei /generate-game / /generate-mappe / /resume-state / Phase-Wechsel / Konsistenz-Check. 6-Schritte-Pre-Flight-Prozedur (Phase-Stand → Ziel-Phase → Transition-Lookup → Pre-Conditions auswerten → Aggregation → Output). 3 Beispiel-Pflicht-Szenarien. Override-Mechanismus verankert. **B.16 (1 PT, F-D2-04 + F-PA-05):** NEU tools/promote_sequenzkontext.py (~210 LOC). CLI mit --mappe-root/--dry-run/--verbose/--self-test. Pattern: sequenzkontext-mat-N-M.json → mappe_root/materialien/mat-N-M/sequenzkontext.json. Idempotent. Self-Test 3/3 PASS + Idempotenz rc=0. **F-PA-05 (mappe-[N]-Resolver MED-deferred)** IMPLIZIT GESCHLOSSEN. **Smoke:** Validator 0 Errors, alle Self-Tests PASS, Skill-Discovery PASS. **Track-P.2-Endstand: 31/31 PT (100%) Phase A + B komplett inkl. B.3b Audit-Net.** Phase C (B.13 + B.14, 4 PT) optional/deferrable laut Plan §3.3. **Aufwand-Ist Phase A+B Total: ~5.5h Wall-Clock** statt Plan-Schaetzung ~3 Wochen (massiv beschleunigt durch Subagent-Pattern + Self-Edit-Fallback + Parallel-Dispatch). **Akzeptanzkriterien Track P.2:** alle 10 Hart-Kriterien erfuellt (Splits + State + Transitions + Hooks + Skills + Tools + Pre-Flight + Pfad-Konsolidierung + dispatch_meta + Bundle-Audit). Gen-Commit `7666b2d`. Vorheriger Eintrag — **Track P.2 Phase B B.8 + B.11 DONE (parallele Subagent-Dispatch, 4 PT, ~35 Min Wall-Clock)** — Zwei general-purpose-Subagents im selben Message-Block dispatched, kein File-Ownership-Konflikt. **B.8 (2 PT, F-D4-02):** NEU architektur/schemata/aufgabe_v1.json (95 Z., $id=aufgabe_v1, 8 type-enums + bloom L1-L6 + afb I-III), NEU tools/validate_aufgabe_output.py (250 LOC), NEU tools/check_aufgabe_regex.py (305 LOC, 4 Checks A-M16/A-M17/Bloom/Loesung-Format). 2 NEUE Hooks: pre-write-aufgabe (D.5) + post-write-aufgabe (D.6) analog Material-Hooks D.1+D.2. **B.11 (2 PT, F-D2-02 + F-PA-05 + F-PA-10 + F-PB7-01):** 8 File-Edits zu Triple-Root-Pfad-Konvention. agent-material-dispatcher + agent-raetsel-dispatcher um Sektion "Subagenten-Dispatch-Konvention" mit dispatch_meta-Pflicht-Format ergaenzt (F-PB7-01 GESCHLOSSEN). resume-state.md F-PA-10 Fix: 3-stufiger Glob-Pattern (Triple-Root primaer + generischer Fallback + Legacy). 5 Commands auf {{TARGET_PATH}}-Konvention. **Verifikation:** grep -r /Users/paulad/ → 0 Treffer, grep mappe-[N] → nur phase_transitions.json + ORCHESTRATOR out of scope, 3 Plugin-Specs mit {{TARGET_PATH}}. **Smoke:** Validator 0 Errors, alle Self-Tests PASS, 6/6 Hook-Smoke PASS. **Track-P.2-Stand: Phase A 18.5 + Phase B 10/12.5 = 28.5/31 PT (92%).** Verbleibend Phase B: B.12 (1.5 PT) + B.16 (1 PT) = 2.5 PT. Phase C: 4 PT. Parallel-Subagent-Speedup ~40% (35 Min statt 60 Min sequenziell). Gen-Commit `a6d354d`. Vorheriger Eintrag — **Track P.2 Phase B Continue — B.9 + B.10 DONE Bundle (Self-Edit, 2 PT)** — Both Self-Edit-Hauptthread, kein Subagent. **B.9 (1 PT, F-D4-03):** NEU tools/assembly_verify_runner.py (~280 LOC, V13-V20-Batch lightweight: V13 JSON-Syntax / V14 Required-Keys / V15 mappen non-empty / V16 mappe-id+materials+aufgaben / V17 freischalt_codes-Count / V18 duplicate-IDs / V19+V20 non-zero Counts als Warning, Exit 0/1/2/3, --self-test 4/4 PASS). NEU Hook post-assembly-verify (PostToolUse Write, Filter escape-games/*/data.json + games/*/data.json, exit 1 als Warning). **B.10 (1 PT, F-D2-01+F-D6-04+06):** NEU tools/code-mode-launch.sh (~60 Z. Bash-Wrapper Triple-Root-Add-Dir + Plugin-Install-Pruefung + MCP-Reminder), commands/generate-game.md Pre-Flight-Sektion erweitert (Code-Mode-Launch-Konvention + MCP-Pflicht-Connectoren wikipedia/wikimedia-image-search + 6-Schritte-Pre-Flight), README.md Setup-Sektion 4 NEU. **Smoke:** Validator 0 Errors, assembly_verify_runner Self-Test 4/4 PASS, code-mode-launch.sh syntax OK. **Track-P.2-Stand: Phase A 18.5 + Phase B 6/12.5 = 24.5/31 PT (79%).** Verbleibend Phase B: B.8 (2 PT) + B.11 (2 PT) + B.12 (1.5 PT) + B.16 (1 PT) = 6.5 PT. Phase C: B.13 (3 PT) + B.14 (1 PT). Aufwand-Ist B.9+B.10: ~25 Min im Plan-Korridor 2 PT. Gen-Commit `cb37601`. Vorheriger Eintrag — **Track P.2 Phase B Start — B.15 + B.7 DONE im Bundle (4 PT)** — **B.15 (1 PT, F-D4-01)** Hook-Pattern-Refactor: D.1+D.2 differenzieren material.json (primary, blockt) vs material_v*.json (revisor, loggt nur). **B.7 (3 PT, F-D4-04+05)** via Cowork-Task-Tool-Subagent (~25 Min). 3 NEUE Files: architektur/schemata/dispatch_meta_v1.json (80 Z. JSON-Schema), tools/dispatch_meta_helper.py (380 LOC, Atomic-Write tempfile+os.rename+fsync, fcntl-Lock graceful-degrade, --update mit type-coercion, --append-finding, --self-test 4/4 PASS), tools/check_q_gate_log.py (238 LOC, Regex-Pattern Phase X.Y, Status-Marker PASS/FAIL/WARN/NOT_FOUND, --edit-diff stdin-Modus, --self-test 6/6 PASS). 2 NEUE Hooks in hooks/hooks.json: pre-pi-phase-advance (PreToolUse Edit, regex-extrahiert Phase aus old/new_string, prueft Q-GATE-LOG via check_q_gate_log.py, PASS/NOT_FOUND=allow/skip, FAIL/WARN=block exit 2) + post-material-subagent-stop (PostToolUse Task, Filter sub-material-* + dispatch_meta=<path>-Konvention, --update stopped_at). **Naming-Klarstellung:** Plan-D.3+D.4-IDs durch B.3+B.4 bereits belegt → beschreibende IDs verwendet. **Smoke:** Validator 0 Errors, alle Self-Tests PASS, Hook-Smoke-Tests PASS. **Open Convention-Gap (F-PB7-01 LOW):** dispatch_meta=<path>-Konvention muss in agent-material-dispatcher.md spezifiziert werden, opportunistisch in B.11. **Track-P.2-Stand: Phase A 18.5 + Phase B 4/12.5 = 22.5/31 PT (73%).** Naechster Block: B.8 D.5+D.6 Aufgaben-Hooks (2 PT) / B.9 SUB_ASSEMBLY_VERIFY (1 PT) / B.10 Pre-Flight-Doku (1 PT) — alle parallelisierbar. Aufwand-Ist B.15+B.7: ~30 Min unter Plan 4 PT. Gen-Commit `bead357`. Vorheriger Eintrag — **Track P.2 Phase A Bundle-Audit + B.3b Pflicht-Fix-Cycle DONE — Phase A komplett-net 18.5/18.5 PT (100%)** — Bundle-Audit via Cowork-Task-Tool-Subagent (~50 Min Wall-Clock) liefert 3 HIGH / 4 MED / 8 LOW Aggregat (BEFUND_PHASE_A_BUNDLE_AUDIT.md persistiert ~180 Z.). Aggregat-Urteil PASS-mit-Pflicht-Fix. **B.3b Pflicht-Fix (~30 Min Wall-Clock):** **F-PA-01 (HIGH)** 4 agent-recon-Phantom-Refs ersetzt durch reale Plugin-Subagents (agent-didaktik 0.1, agent-inhalt 0.2, agent-medienrecherche 0.2.M, agent-skript 0.3, agent-hefteintrag 0.4). **F-PA-02 (HIGH)** game_state.schema.json `_meta.properties.override_reason` als optionaler String ergaenzt + SKILL.md Override-Mechanismus auf Schema-Realitaet abgeglichen. **F-PA-03 (HIGH)** phase_transitions.json um 8 fehlende Transitions erweitert (10->18 Transitions): 0.2->0.2.M, 0.2.M->0.3, 1->2.0, 2.0->2.0b, 2.1->2.1b, 2.1b->2.1c, 2.1c->2.2a, 2.2c->3.0, 3.0->3.1, 3.1->3.2, 3.2->DONE. Schema phase-enum 12->19 Werte. SKILL.md Phase-Reihenfolge aktualisiert. **F-PA-04 (MED)** tafelbilder-Pre-Cond geaendert auf `file_exists:TAFELBILD_Mappe1.md`. **Smoke:** phase_transition_runner.py --self-test exit 0 mit 18 Transitions, Validator 0 Errors. **Phase-B-Risiko-Status:** B.7 + B.10 + B.11 + B.12 (HIGH-blockiert vor B.3b) → ENTBLOCKT. Verbleibend: 4 MED + 8 LOW (nicht-blockend, opportunistisch in B.7+ Cleanup). **Track-P.2-Phase-A-Stand: 18.5/18.5 PT (100%) komplett-net.** Naechste Phase: Phase B (8 parallele Bloecke, 12.5 PT). Final-Commit Gen-Repo `59636aa`. Vorheriger Eintrag — **Track P.2 Phase A KOMPLETT (17/17 PT) — B.5 + B.6 DONE im Bundle** — Self-Edit-Hauptthread (kein Subagent). **B.5:** commands/resume-state.md erweitert auf JSON-First-Probe via Glob `game-*/game_state.json` + Pfad-Selektion via `_meta.last_updated`-Maximum + Schritte 2a-4a JSON-State-Lesen + validate + render + 6-Felder-Extraktion + phase_transitions.json-Lookup + Konsistenz-Check JSON vs PI + Cross-Refs auf B.3+B.4. PI-Fallback-Logik (Schritte 2b-4b) BEHALTEN. **B.6:** NEU skills/state-advance-policy/SKILL.md (~6 kB) — Auto-Trigger bei Phase-Advance/State-Write/Resume/Q-Gate/Konsistenz-Checks. Body: 10-Phasen-Reihenfolge + 4 Pre-Condition-Types (file_exists/directory_populated/q_gate_log_pass/json_schema_valid) + 6 Wann-Verboten-Conditions + Q4 Option-C+ Konfliktloesung + Hook D.3+D.4-Integration + Override-Mechanismus mit `_meta.override_reason`-Pflicht + Q-Gate-LOG-Konvention + 11 Cross-Refs. **Phase-A-Bilanz:** B.1 (5 PT) + B.2 (3 PT) + B.3 (3 PT) + B.4 (3 PT) + B.5 (1 PT) + B.6 (2 PT) = 17/17 PT (100%). **Aufwand-Ist Phase A gesamt: ~2.5h Wall-Clock** unter Plan-Schaetzung 17 PT (~8.5 Plan-PT pro Stunde durch Subagent-Pattern + Self-Edit-Fallback). **Smoke:** Validator 0 Errors, Skill-Discovery Plugin-Auto-Discovery sieht skills/state-advance-policy/SKILL.md, Cross-Ref-Konsistenz PASS. **Naechste Phase:** Phase B (8 parallele Bloecke, 12.5 PT, ~1.5 Wochen) — B.7 Hook-Coverage + B.8 Pre-Flight + B.9 Pfad-Resolution + B.10 Atomic-Writes + B.11 Test-Fixtures + B.12 Parallel-Dispatch. Final-Commit: Gen-Repo `4e5a16d`. Vorheriger Eintrag — **Track P.2 Block B.4 DONE — game_state.json Schema + Validator + Render + Hook D.3** — 4 Deliverables: NEU schemas/game_state.schema.json (128 Z., JSON-Schema draft 2020-12, 12 phase-enums incl DONE, mappe-[N]-Pattern, additionalProperties:false strict), NEU tools/validate_game_state.py (241 LOC, graceful-degrade-Pattern, exit 0/1/2, --self-test PASS+FAIL-Mocks), NEU tools/render_game_state.py (244 LOC, deterministic JSON->Markdown-PI-Mirror-Renderer, idempotent via _meta.last_updated, --self-test 8/8 PASS), MODIFIED hooks/hooks.json D.3 post-state-update aktiviert (PostToolUse Write|Edit|NotebookEdit, jq-Pfad-Filter *game_state.json, validate->render->.cache/last_pi_mirror.md, timeout 5s, exit 1 als Warning). Bonus NEU .cache/.gitignore. **Q4 Option-C+ (JSON-authoritativ + PI-Mirror + Sync-Hook) strukturell umgesetzt.** F-B3-02 implizit geloest via schema mappen[i].id `^mappe-[0-9]+$` als kanonische Quelle. **Smoke:** alle Self-Tests PASS, End-to-end Mock-State validate+render PASS, Hook-Smoke-Test PASS, Validator 0 Errors. **Track-P.2-Phase-A-Stand: 14/17 PT (82 %).** Naechster Block: B.5 `/resume-state` JSON-Pfad-Erweiterung (1 PT). Aufwand-Ist: ~25 Min Wall-Clock unter Plan-Schaetzung 3 PT. Gen-Commit `2352234`. Vorheriger Eintrag — **Track P.2 Block B.3 DONE — phase-transition-runner + phase_transitions.json + Hook D.4 opt-in** — 3 Deliverables: NEU agents/phase_transitions.json (4.9 kB, 10 Transitions ONBOARDING→0.1→0.2→0.3→0.4→1→2.0b→2.1→2.2a→2.2b→2.2c, pro Entry pre/post-conditions + responsible_agent), NEU tools/phase_transition_runner.py (17.1 kB, 445 LOC, CLI mit --self-test/--from/--to/--state-root, 4 condition-types file_exists/directory_populated/q_gate_log_pass/json_schema_valid, Exit 0/2/1, graceful-degrade), MODIFIED hooks/hooks.json (D.4 opt-in via *.phase_transition_hint.json, exit 2 blockt Tool-Call bei FAIL). **Smoke:** --self-test exit 0 (10 Transitions), FAIL-path exit 2 verifiziert, Validator 0 Errors. Deferred-Findings F-B3-01 (agent-recon Placeholder Phase 0.1-0.4) + F-B3-02 (mappe-[N]-Resolver) beide LOW. **Track-P.2-Phase-A-Stand: 11/17 PT (65 %).** Naechster Block: B.4 game_state.json Schema + Validator + Sync-Hook + Render-Tool (3 PT). Aufwand-Ist: ~50 Min Wall-Clock im Plan-Korridor (3 PT). Gen-Commit `6511439`. Vorheriger Eintrag — **Track P.2 Block B.2 DONE — AGENT_RAETSEL 3-way-Split** — AGENT_RAETSEL.md ~25.7 kB aufgeteilt: agent-raetsel-progressionsplan.md (10.5 kB, Phase 2.2a Progressionsplan + Bloom-Verteilung + Operationalisierungsziel) + agent-raetsel-dispatcher.md (20.3 kB, Phase 2.2b Konstruktionskontext + Task-Tool-Dispatch + Cross-Aufgaben-Konsistenz + Assembly v4). Legacy-Marker DEPRECATED-Header in alter AGENT_RAETSEL.md. Cross-Ref-Update in 21 Files (~42 Edits, 8/8 SUB_AUFGABE_* + AGENT_DIDAKTIK/INHALT/SKRIPT/HEFTEINTRAG/ARTEFAKT + PROJECT_INSTRUCTIONS.md). **Smoke-Verifikation:** Validator PASS (1 unrelated Warnung), Outline `## Aufgaben`-Parent in beiden Files (kein analoges F-D1-01), 8/8 SUB_AUFGABE-Cross-Refs PASS, RP2-1-Risk (Cross-Ref-Verlust) NICHT eingetreten. Subagent-Pattern empirisch validiert via B.1 → kein Full-Audit-Subagent (Mini-Smoke ausreichend, ~10 Min Audit-Cost-Saving). **Track-P.2-Phase-A-Stand: 8/17 PT (47 %).** Naechster Block: B.3 phase-transition-runner + phase_transitions.json (3 PT). Aufwand-Ist: ~20 Min Wall-Clock statt 3 PT-Schaetzung. Final-Commit pending. Vorheriger Eintrag — **Track P.2 Block B.1 DONE — AGENT_MATERIAL 3-way-Split** — 1098-Z. AGENT_MATERIAL.md aufgeteilt: agent-material-design.md (Phase-1-Design) + agent-material-dispatcher.md (Phase-2.0b-2.1 Produktion). Gate-Runner-Komponente verteilt auf D.1+D.2-Hooks + reviewer-material-quellentext + dispatch_meta_helper.py (B.7-pending). Cross-Ref-Update in 18 von 20 Pflicht-Files (kontextabhaengig design/dispatcher/both). Legacy-Marker DEPRECATED-Header in alter AGENT_MATERIAL.md. **Bonus-Validator-Fix:** marketplace.json + hooks.json `$schema`+`_note` Top-Level-Keys entfernt (Validator strict). Validator: ✔ Validation passed with warnings (0 Errors). Track-P.2-Phase-A-Stand: 5/17 PT (29%). Naechster Block: B.2 AGENT_RAETSEL Split (3 PT analog). Vorheriger Eintrag — **Architektur-Refresh-Audit DONE + Track-P.2-Plan-Doku v1.0 persistiert — Track P.2 ready zum Start** — **Audit-Befund:** 29 Findings (13 HIGH + 16 MED + 0 LOW) in 6 Dimensions (Subagent-Dispatch / File-Pass-Through / State-Sync / Hook-Coverage / Cross-Subagent-Komm / Self-Sustained-Run-Boundaries). Kritische HARTE BRUECHE in Phase 1+2.1+2.1c+2.2a wegen AGENT_MATERIAL+AGENT_RAETSEL Phase-2-deferred. **BEFUND_ARCHITEKTUR_REFRESH_PRE_P2.md v1.0** persistiert. **Track-P.2-Plan-Doku C-TIEFE-REFACTOR-PLAN.md v1.0** persistiert: 16 Bloecke (Phase A 6 sequenziell + Phase B 8 parallel + Phase C 2 Optimierung), 33.5 PT Total, ~5 Wochen. Phase A kritischer Pfad: B.1 AGENT_MATERIAL-3-way-Split → B.2 AGENT_RAETSEL → B.3 phase-transition-runner → B.4 game_state.json + Sync-Hook → B.5 /resume-state-Erweiterung → B.6 state-advance-policy-Skill. Phase B parallelisierbar (Hook-Coverage + Pre-Flight + Pfad-Resolution). Phase C Optimierung (11 Vertrags-Skills + V16-Refactor 47k Token-Einsparung). Akzeptanzkriterien: T-P.2-Smoke + T-State-Recovery + A-Plugin-Phase-2 + Plugin-Readiness >=95%. Roadmap v2.2 -> v2.3 mit Plan-Doku-Verweis. T-Full-Game-Pilot NACH Track P.2. User-Reasoning empirisch bestaetigt: Pilot frueher als nach Phase A unsinnig (HARTE BRUECHE). Final-Commit pending. Vorheriger Eintrag — **F-S-01 Fix DONE — Plugin-Setup-Doku + Tool-Resilient + jsonschema-Install verifiziert** — 4 Files: NEU `requirements.txt` (jsonschema>=4.0) / NEU `tools/setup-deps.sh` (One-Shot-Install mit Pre-Check + Verifikation) / NEU `README.md` (Plugin-Setup + Strict-Separation + Komponenten-Inventar + Triple-Root + Dev/Prod-Workflow) / `tools/validate_material_output.py` resilient mit graceful-degrade-Branch (jsonschema-Missing -> basic JSON-Parse + Warning + exit 0 + degraded_mode-Flag, Hook D.1 blockt nicht). Empirisch verifiziert: `python3 -m pip install jsonschema` PASS / Tool-Run mit jsonschema PASS (full Schema-Validation) / Tool-Run ohne jsonschema PASS-DEGRADED (basic JSON-Parse + Warning) / Setup-Skript bash-konform. Pilot-Block AUFGEHOBEN. Final-Commit pending. Vorheriger Eintrag — **T-P.1-Smoke S3+S4+S5 Empirie DONE — 3/4 PASS + 1 KRITISCHES FINDING jsonschema-Dependency** — Limited-Smoke-Suite via Code-Mode-CLI headless: T-A Plugin-Re-Install PASS / **T-B Command /resume-state PASS** (Plugin-Prefix /escape-game-generator:resume-state, LLM las PI-Zustandsblock + State-Bericht generiert) / **T-C Skill f0b-priming PASS** (auto-getriggert bei Material-Phase-Prompt, alle 6 Sprachniveau-R7-Grenzen + DaZ-Pflicht + Konfig-SSoT-Refs im Output) / **T-D Hook D.1 MIXED** (Hook-Mechanik PASS, aber NEUER HIGH-Finding F-S-01: validate_material_output.py braucht `jsonschema`-Python-Modul, ModuleNotFoundError). T-E Cleanup PASS (Plugin uninstalled + marketplace removed, Strict-Separation-Restore). **NEUER HIGH-Pflicht-Fix vor Pilot-Einsatz:** Plugin-README + Plan-Doku §2.4 + PLUGIN_ARCHITEKTUR §6 muessen Python-Dependencies-Setup dokumentieren (`pip install jsonschema`) ODER validate_material_output.py resilient machen mit graceful-degrade. **Akzeptanz-Update:** Plugin-Loader + Skill-Trigger + Command-Aufruf + Hook-Mechanik empirisch verifiziert. Pilot-Einsatz BLOCKIERT bis F-S-01 gefixt. Final-Commit pending. Vorheriger Eintrag — **A-PLUGIN-PHASE-1 AUDIT DONE — PASS-mit-Pflicht-Fix, Plugin-Readiness 88%** — Statisches 3-Dim-Audit (Block-Konformitaet + Cross-Repo+Doku-Konsistenz + Phase-1-Akzeptanz-Komplettheit) durch Cowork-Task-Tool-Subagent. **BEFUND_A_PLUGIN_PHASE_1.md v1.0** persistiert. **Aggregat: HIGH=1 / MED=3 / LOW=3**. Plan §4 8/8 Hart-Akzeptanzkriterien strukturell erfuellt. Plugin-Readiness 3% -> ~88% (uebersteigt Plan-Threshold 85%). **HIGH-Pflicht-Fix DONE:** F0e_PLUGIN_ARCHI_AUDIT_BEFUND.md v1.1 -> v1.2 mit Status-Korrektur-Tabelle (F-B-01 + F-B-02 RE-BESTAETIGT-HIGH nach Plugin-Loader-Empirie + Lesson-Learned). **MED-Pflicht-Fix DONE:** Plan-Doku §2.3 auf v0.3.1-Stand (Manifest minimal ohne agents/commands/hooks/skills-Globs, Auto-Discovery, Lesson-Learned-Block). **MED+LOW-Findings deferred:** F0B-Marker-Removal (Sub-B.1b Phase 2) + D.3/D.4 Helper-Tools (Sub-D-Phase-2) + skills/README.md-Update (LOW, opportunistisch). **Phase-1-Akzeptanz-Empfehlung: PASS-mit-Pflicht-Fixes DONE.** Track P.1 ready fuer Phase-2-Start. Final-Commit pending. Vorheriger Eintrag — **STRICT-SEPARATION verankert — Cowork=Plugin-Dev / Code-Mode=Plugin-Prod (self-sustained)** — User-Architektur-Direktive 2026-04-25 (post-T-P.1-Smoke). Q7 verschaerft von "Code-Mode = Prod" zu "AUSSCHLIESSLICH-Code-Mode-Prod". Plugin-Install in Cowork = Anti-Pattern (Skills/Hooks triggern bei Source-Editing kontraproduktiv). 5 Updates: PLUGIN_ARCHITEKTUR_TRIPLE_ROOT.md §3 verschaerft + neue §3.4 Anti-Pattern + §3.5 Verifikations-Pfad-Tabelle / PROJECT_INSTRUCTIONS.md RUNTIME-KONVENTION-Sektion verschaerft mit Plugin-geladen-Spalte / BEFUND_T_P1_SMOKE.md §5 Naechste-Schritte praezisiert (S3-S7 Code-Mode-Pflicht) / TEST_AUDIT_ROADMAP.md v2.1 -> v2.2 mit Strict-Separation-Klarstellung / NEUE Auto-Memory feedback_runtime_strict_separation.md. **Cleanup ausgefuehrt:** Plugin in Cowork uninstalled + Marketplace removed. Cowork-Mode operiert nun auf Source-Files only. Final-Commit pending User-Freigabe. Vorheriger Eintrag — **T-P.1-Smoke S2 PASS post-Fix-Cycle — Plugin Status enabled** — Smoke-Setup: Marketplace-Wrapper `.claude-plugin/marketplace.json` mit url-source via HTTPS-URL (commits db72014 + f36ff5f). Initial-Plugin-Install zeigte 4 HIGH-Errors die `claude plugin validate` NICHT detektiert hat: agents/commands-Globs `Path not found` + hooks-Feld `Duplicate hooks file detected`. **WIDERLEGT damit Light-Audit-BEFUND v1.1 §13 F-B-01/F-B-02 WIDERLEGUNG** — community-Sekundaerquelle PLUGIN_SCHEMA_NOTES.md hatte KORREKT die Anti-Patterns markiert. Validator-CLI ist NICHT voll-empirisch. Plugin-Loader-Run ist autoritative Quelle. Smoke-Fix-Cycle (commit b6bf35f): Manifest v0.3.0 -> v0.3.1 mit agents/commands/hooks-Feldern ENTFERNT (Auto-Discovery via Konvention). Re-Install: **Plugin Status `✔ enabled`**. Cache-Inventar verifiziert: 5 Skills + 6 Commands + 10 Agents. **BEFUND_T_P1_SMOKE.md v1.0 persistiert** mit 4-Lessons-Learned. S3-S7 (Skill-Auto-Trigger + Hook-Trigger + Command-Aufruf + E2E) DEFERRED zu dedizierter Code-Mode-Session (LLM-Token-Run noetig). Auto-Memory-Update `feedback_validator_cli_primary.md` mit Validator-CLI-not-sufficient-Erkenntnis. Final-Commit pending User-Freigabe. Vorheriger Eintrag — **TRACK P.1 KOMPLETT DONE — Block H DONE, alle 8 Bloecke A-H abgeschlossen** — Block H: NEU `docs/projekt/PLUGIN_ARCHITEKTUR_TRIPLE_ROOT.md` v1.0 (Triple-Root-Topologie + Plugin-Variable-Path-Resolution + Cowork-vs-Code-Mode-Discovery + Read-Only-Scope 3.-Root + Cross-Repo-Workflow-Pattern + Implementations-Status Phase 1 + Phase-2-Pre-View) + PROJECT_INSTRUCTIONS.md DUAL-ROOT-Sektion -> TRIPLE-ROOT-Sektion erweitert + neue RUNTIME-KONVENTION-Sektion (Cowork=Dev / Code-Mode=Prod, User-Decision Q7) + Verweise auf neue Doku-Datei. Validator: ✔ Validation passed with warnings (0 Errors). **Plan-Stand: 21.5 PT / 21.5 PT (100%)**. Track P.1 ready fuer Akzeptanz-Test (T-P.1-Smoke + A-Plugin-Phase-1, deferred zu Cowork-Plugin-Reload-Cycle in naechster Session). Vorheriger Eintrag — **Track P.1 Block G DONE — 5 Test-Fixtures Top-5 Worker** — Plan §2.7 Konservativ-Layer: Defekt-Fixtures + Smoke-Test-Patterns dokumentiert. G.1 SUB_MATERIAL_QUELLENTEXT (Re-Doku via C1-C3 BEFUNDe, Waterberg-SQ-1 + Herero-Q10-Drift), G.2 SUB_MATERIAL_BILDQUELLE (NEU MV2-Hallu-Fixture mit synthetischer Bundesarchiv-ID `Bundesarchiv_Bild_999-XYZ12345`), G.3 SUB_AUFGABE_MC (NEU Distraktor-Implausibilitaet — Anachronismen + Lange-Asymmetrie 23x), G.4 SUB_AUFGABE_LUECKENTEXT (Pool-Reset-Bug F-RA3-01 historisch, GEFIXED 2026-04-18 commit a4f8c19, Smoke-Pattern dokumentiert), G.5 SUB_ASSEMBLY_VERIFY (NEU V14-Entity-Hallu mit fixture_v14_entity_hallu.json + entities_reference.json). 11 Files in scratch/fixtures/, 5 JSON-Fixtures alle JSON-valid. Phase 1 Strategie: Fixture-Inventar + Smoke-Test-Spec; Voll-Aktivierung CI-Smoke-Test-Run pro PR ist Phase-2-Scope. Validator: ✔ Validation passed with warnings (0 Errors). Aufwand-Ist: ~15 Min Wall-Clock fuer 11 Files. Final-Commit pending User-Freigabe. Plan-Stand: A+B+C+D+E+F+G DONE (20.5 PT / 21.5 PT, ~95%). Vorheriger Eintrag — **Track P.1 Block F DONE — AGENT_QUALITAET Drift-Cleanup** — 4 Drift-Refs auf Legacy AGENT_DESIGN/AGENT_TECHNIK in agents/AGENT_QUALITAET.md mit Legacy-Marker versehen (Z.20: Output-Verweis, Z.116: Design-Spezifikation strikethrough + Verweis auf assets/css + Engine-Assembly, Z.155-156: Issues-Tabelle Zustaendiger-Agent-Spalte). grep-Smoke-Test PASS: keine unmarkierten Refs verbleibend. Validator: ✔ Validation passed with warnings (0 Errors). Aufwand-Ist: 0.5 PT laut Plan. Final-Commit pending User-Freigabe. Plan-Stand: A+B+C+D+E+F DONE (16.5 PT / 21.5 PT, ~77%). Vorheriger Eintrag — **Track P.1 Block E DONE — 6/6 Slash-Commands** — /generate-game (Phase 0-2 vollstaendig + Subagent-Dispatch-Sequenz pro Phase 0.1-0.4 + 2.1-2.2c), /generate-mappe (Phase 2.0+ einzelne Mappe), /resume-state (Compaction-Recovery aus PI-Zustandsblock + Q4-Option-C+ Phase-2-Hinweis), /validate-game (Q-Gate-Aggregat: Schema G1 + Regex M16/M17 + Source-Deploy-Parity + Sequenzkontext + Trigger-Sichtbarkeit + V13-V20), /audit-game (Multi-RA-Audit-Pattern analog Testrun-N-K mit RA1-RA5-Kanon + Audit-Persistenz-Best-Practice), /migrate-legacy (Schema-Migration v3.10.x->v3.10.4 via tools/migrate_material_v3_10_2_to_v3_10_4.py). Command-Format empirisch via ui-design-Plugin verifiziert: Frontmatter description + argument-hint + Body-Prompt mit `$ARGUMENTS`. Konservativ-Layer: Commands sind Aufruf-Trigger, Workflow-Logik in PROJECT_INSTRUCTIONS.md/AGENT_*-Files. Bonus: commands/README.md mit Frontmatter (INTERNAL DOC) ergaenzt -> Validator-Warning behoben (7/8 Warnings, 1 weniger). Validator: ✔ Validation passed with warnings (0 Errors). Aufwand-Ist: 4 PT (laut Plan, manuelle Iteration). Final-Commit pending User-Freigabe. Vorheriger Eintrag — **Track P.1 Block D DONE Reduziert — D.1+D.2 aktiv, D.3+D.4 Stubs** — Reduzier-Strategie nach User-Decision: D.1 pre-write-material (G1-Schema-Gate, PreToolUse Write, Pfad-Filter materialien/*.json, Block via exit 2) + D.2 post-write-material (M16+M17 Regex-Gates, PostToolUse Write, FAIL-Warning via exit 1) AKTIV. D.3 post-subagent-stop-dispatcher + D.4 pre-phase-advance als _planned_hooks_phase_2 dokumentiert (Helper-Tools dispatch_meta_helper.py + check_q_gate_log.py NEU anzulegen, Phase 2). Hook-Format empirisch verifiziert via everything-claude-code-Plugin (PreToolUse/PostToolUse-Schema mit matcher + hooks-Array + type=command + timeout). Hook-Commands inline-bash mit jq + Python3-Tool-Calls + ${CLAUDE_PLUGIN_ROOT}-Variable. Validator: ✔ Validation passed with warnings (0 Errors, 8 Warnings unveraendert). Aufwand-Ist: 1.5 PT statt 3 PT laut Plan. Final-Commit pending User-Freigabe. Vorheriger Eintrag — **Track P.1 Block B.3+B.4+B.5 DONE — 5/5 Skills komplett** — pfad-manifest (Triple-Root-Architektur + Quellpfade), trigger-sichtbarkeit (STR-12 Lehrkraft-only-Policy + Assembly-Split-Mechanik), rollen-katalog (STR-01 R1-SuS/R2-LK/R3-AUT + Rollen-Matrix). Konservativ-Layer: Source-Files agents/PFAD_MANIFEST.md, agents/POLICY_TRIGGER_SICHTBARKEIT.md, agents/ROLLEN_KATALOG.md UNVERAENDERT (Cross-Refs stabil). Validator-Smoke: ✔ Validation passed with warnings (8 Warnings unveraendert; 5 Phase-2-pending agents-Files + 1 commands/README + 2 deferred). **Block B komplett: 5/5 Skills angelegt.** Final-Commit pending User-Freigabe. Vorheriger Eintrag — **Track P.1 Block B.1+B.2 DONE — 2 Skills + Manifest v0.3.0** — Konservativ-Layer-Strategie: f0b-priming-Skill + escape-game-schema-Skill als ZUSATZ angelegt, F0B-Marker-Bloecke in 22 Subagents BEHALTEN (Hash-Pruef-Mechanismus V16 + sprachniveau_include_registry stabil), ORCHESTRATOR.md UNVERAENDERT (Cross-Refs stabil). Token-Einsparung 47k DEFERRED zu separatem Sub-Block-B.1b. Skill-Format empirisch verifiziert via ui-design-Plugin: skills/&lt;name&gt;/SKILL.md mit Frontmatter `name`+`description` (keine tools/model). Manifest-Glob `skills`-Feld ENTFERNT zugunsten Auto-Discovery (analog ui-design-Plugin). Manifest-Bump v0.2.1 -> v0.3.0. **Validator: ✔ Validation passed with warnings** (8 Warnings unveraendert: 5 Block-B-pending + 2 Phase-2-deferred + 1 commands-README Stub). Final-Commit pending User-Freigabe. Vorheriger Eintrag — **Track P.1 Block A+C DONE inkl. Light-Audit + Validator-Fix-Cycle — Validator PASS** — Light-Audit-Subagent fand 2 HIGH-Findings, post-Audit `claude plugin validate`-Run revidierte 3 Audit-Befunde + fand 3 weitere HIGH (author Object Pflicht, repository String Pflicht, tools-Feld Anti-Pattern, description-YAML-Quoting Pflicht). Validator-Fix-Cycle: plugin.json v0.2.0 → v0.2.1 (author Object zurueck, repository String, tools-Feld entfernt) + 25 Frontmatter-description-Lines single-quoted (24 Block-A + reviewer-material-quellentext.md C1-merged hatte SDK-Strict-Verschaerfung-FAIL) + 4 MED-Findings opportunistisch mit-gefixed (F-A-14 INHALT Phase 0.1->0.2, F-A-19 ARTEFAKT Phase 0.3 nach AGENT_INHALT, F-A-12 QUELLENKRITIK Q-Gate-Set A1-A26 + A27, F-A-08 LUECKENTEXT escape-engine-Z.2798 zu generischer Pool-Reset-Vertrag) + 2 LOW (F-A-10 BEGRUENDUNG Bloom L5, F-A-11 VERGLEICH Bloom L4) + Plan-Doku §2.3 Validator-Format-Regeln + BEFUND v1.0 -> v1.1 mit WIDERLEGT-Markern + 3 NEUE Findings (F-B-06/07/08). **Validator-Smoke-Test PASS**: 0 errors, 8 warnings (alle Phase-2-pending Files: AGENT_MATERIAL/AGENT_RAETSEL deferred Phase 2, F0B_PRIMING_INCLUDE/ORCHESTRATOR/PFAD_MANIFEST/POLICY_TRIGGER_SICHTBARKEIT/ROLLEN_KATALOG werden Block-B Skill-Transformation, commands/README.md Stub-Issue Block-H). Lesson-Learned: Validator-CLI hat Vorrang vor community-empirischer Sekundaerquelle. Final-Commit pending User-Freigabe (5-Stufen-Host-MCP). Vorheriger Eintrag — Track P.1 Block A+C DONE — 24/24 Frontmatter + Plugin-Manifest v0.2.0 — Cowork-Session: Block A.1 (7 SUB_MATERIAL_*) + A.2 (8 SUB_AUFGABE_*) parallel via 2 Cowork-Task-Tool-Agents (~3 Min Wall-Clock) → A.3 (5 Phase-0-Orchestratoren) + A.4 (4 Cross-Concern) self-Edit Fallback nach API-529-Overload (~2 Min) → Block C Plugin-Manifest v0.1.0 → v0.2.0 erweitert (agents/commands/skills/hooks/tools-Globs + license + repository + keywords) + commands/+skills/-Stub-Verzeichnisse mit .gitkeep+README. **Smoke-Test PASS:** 24/24 Frontmatter (alle name kebab-case, tools `Read,Grep,Glob,Write`, model `opus`), JSON-Valid, 5/5 Tools existieren, agents-Glob matched 32 Files. git diff: 25 modified + 4 new = 29 Files in Block-A+C-Scope, 188 insertions, 5 deletions (Manifest-Replace). Akzeptanzkriterium A+C teil-erfuellt — empirischer Plugin-Reload-Discovery-Test braucht Cowork-Plugin-Reload-Cycle. Final-Commit pending User-Freigabe (5-Stufen-Host-MCP). Vorheriger Eintrag — **C-ARCHI-AUDIT inkl. User-Review-Gate DONE — Track P.1 READY** — User-Decisions Q1-Q9 integriert in BEFUND v1.1 + Recon v1.3 + Roadmap v2.1 + neuer C-INKREMENTELL-MIGRATION-PLAN.md v1.0. **Pfad H1→H2-Staffel bestaetigt.** Q4 PI-Zukunft entschieden: **Option C+** (JSON-authoritativ + PI-Mirror + Sync-Hook). Q7 Code-Mode = Prod / Cowork = Dev offiziell verankert. Q5+Q6 Defaults mit Evaluations-Spike. Q2 Triple-Root Read-Only-Scope formalisiert. Q8+Q9 Recon-Drift + Roadmap-Markers DONE. **Track P.1 (3 Wochen, 21.5 PT, 8 Bloecke A-H):** YAML-Frontmatter 24 Workers + 5 Skills + Plugin-Manifest + 4 Hooks + 6 Slash-Commands + AGENT_QUALITAET-Drift-Cleanup + 5 Test-Fixtures Top-5 + Cross-Repo-Doku + Code-Mode-Anker. Akzeptanzkriterium: Plugin-Readiness 3 % → ≥85 %. Daily-Rhythm 3-Wochen-Plan in Plan-Doku §6. Final-Commits ausstehend, dann Track-P.1-Start ready. Vorheriger Eintrag — **C-ARCHI-AUDIT S2-S7 DONE — TRACK ABGESCHLOSSEN** — Gen-Repo main `c0600f9` (S6 Roadmap v2.0). Vollstaendige C-ARCHI-AUDIT-Commit-Kette: `d25a627` (R1-R11 Revisionen) → `59575a2` (S2 Inventar) → `7d00253` (S3 7-RA-Dispatch) → `09ab5da` (S4 Aggregation) → `17edf2b` (S5 BEFUND v1.0) → `c0600f9` (S6 Roadmap v2.0). **Pfad-Empfehlung H1→H2-Staffel** (gewichteter Entscheidungs-Matrix-Score 138 vs H2 mono 110). **Phase 1 (3W mechanische Migration):** YAML-Frontmatter 24 Workers + 5 Skill-Transformationen + Plugin-Manifest + 4 Hooks + 6 Commands + Test-Fixtures Top-5 + Cross-Repo-Doc. **Phase 2 (5W strukturelle Refactor):** AGENT_MATERIAL 3-way-Split + State-JSON + 11 Vertrags-Skills + Atomic-Writes + Revisor-Loop-Generalisierung 14 Typen + 13 weitere Fixtures + Parallel-Dispatch. **Total Plugin-Migration: ~8 Wochen.** Roadmap v2.0: Plugin-Migration als neuer Top-Level-Track P. C4-C9 subsumiert in Track P.2. Pre-Pilot + T-Full-Game + Multi-RA-Audit + Pilot-Freigabe + Track D + Lehrkraft-Extern danach. Total bis Pilot-Freigabe: **~10-12 Wochen**. **User-Review-Gate aktiv:** BEFUND v1.0 §10 enthaelt 9 offene Fragen + 11 Audit-Entscheidungen. Bei User-Go: Track P.1 startet. **Akzeptanzkriterien:** 6/6 Hart + 5/5 Weich erfuellt. Vorheriges Update 2026-04-25: **C-ARCHI-AUDIT S2+S3+S4 DONE**.

Vorheriger Eintrag — Gen-Repo main `7d00253` (S3) + ausstehender Commit fuer S4 Aggregation + STATUS+CHANGELOG-Pflege. Y1-Split-Strategie umgesetzt: Review-Revisionen R1-R11 (v1.0→v1.1, Commit `d25a627`) → S2 Inventar-Detail + Recon v1.1→v1.2 (Commit `59575a2`) → S3 7-RA-Multi-Audit-Dispatch via 3×3+1-Sequential-Staging (Commit `7d00253`) → S4 Cross-RA-Aggregation + Gap-Analyse (Aggregation-Artefakt persistiert). **7 valid JSON RA-Reports** in `scratch/archi-audit/RA-P{1..7}-finding-report.json`. **Empfehlungs-Verteilung:** H2=4 (RA-P1/P3/P4/P5), H1=2 (RA-P6/P7), H4=1 (RA-P2). **Confidence-gewichtet:** H2=2.99, H1=1.47, H4=0.65. **Hoechste Einzel-Confidence:** RA-P7 D9 Dual-Root H1 0.82. **HIGH-Prio-Dimensionen-Split:** D1 → H2, D9+D10 → H1. **Vorlaeufige Synthese-Empfehlung (nicht final):** **H2 mit H1-Vorstufe (gestaffelt, ~8 Wochen Total)**. **Per-Subagent-Klassifikation:** STARK-konvergent fuer 18 Worker (direkt-portierbar) + 5 Skill-Kandidaten (als-skill 6/6) + AGENT_MATERIAL (refactor 6/6 Konsens). **42 Architecture-Gaps dedupliziert** zu ~28 unique, davon 14 HIGH-Severity (plugin.json minimal + hooks/commands/skills fehlen + AGENT_MATERIAL-Monolith + PI-State-Machine ohne Plugin-Pendant + Test-Fixture-Defizit + atomic-writes fehlen + State-Schema fehlt + Plugin-Multi-Root-SDK-Status unklar). **Neuer S4-Befund (G-NEU-1, HIGH):** 3.-Root-Topologie entdeckt (`Unterrichtseinwicklung/`, ~1000+ Files, Host-lokal, Nicht-Git) — Recon v1.3-Drift-Korrektur empfohlen. **8 Cross-RA-Patterns** synthetisiert (Plugin-Stub-Widerspruch, Monolithische Orchestratoren, Skill-Hebel-Ungenutzt, State-Asymmetrie, Cowork-vs-Code-Mode-Hartwert, Test-Fixture-Defizit, Legacy-Drift AGENT_QUALITAET, Vertraege-Skill-Konsens). **9 offene Fragen fuer User-Review** in Aggregation-§9 (Staffelung, 3.-Root, AGENT_MATERIAL-Granularitaet, PI-Zukunft, Vertrags-Skill-Granularitaet, Test-Fixture-Scope, Code-Mode-Verankerung, Recon-Drift v1.3, Roadmap-v2.0-Scope). **Naechste Schritte:** S5 BEFUND-Schreibung + Entscheidungs-Matrix-Auswertung (3-4h) → S6 Roadmap v2.0-Revision (2-3h) → S7 PM-Pflege + User-Review-Vorbereitung (1-2h). Aufwand-verbleibend: 6-9h.

Vorheriger Eintrag **C-ARCHI-AUDIT S0+S1 DONE** — Gen-Repo main `62e550c`. Plugin-Architektur-Audit-Track initiiert. `F0e_PLUGIN_ARCHI_RECON.md` v1.0 + `F0e_PLUGIN_ARCHI_AUDIT_PLAN.md` v1.0 persistiert (~928 Zeilen neu). Kritische Recon-Erkenntnisse: Plugin-Readiness nur 3 % (1/30), eigentlicher Orchestrator ist text-basierte State-Machine in PROJECT_INSTRUCTIONS.md, Dual-Root-Architektur (GENERATOR+TARGET) fundamental, 5 Phasen + 17 Sub-Phasen + 30 Agents + 16 Vertraege. Track C1-C3.5 adressierte nur 5-8 % der Gesamt-Architektur. **4 Hypothesen fuer Plugin-Migration: H1 Inkrementell / H2 Tiefe Refactor / H3 Scratch-MVP / H4 Hybrid.** 12 Audit-Dimensionen (inkl. D9 Dual-Root, D10 State-Machine, D11 Vertraege, D12 Cowork-vs-Code-Mode). 7-RA-Kanon (P1-P7 inkl. neuer RA-P6 Phasen-Architektur + RA-P7 Cross-Repo). **C4-bildquelle vertagt** bis Audit-Pfad-Entscheidung. Roadmap v2.0-Revision nach Audit geplant. Aufwand: 17-24h fuer S2-S7 pending. Vorheriger Eintrag **C3.5 Legacy-Migration-Tool DONE** — Gen-Repo main `60b125a`. `tools/migrate_material_v3_10_2_to_v3_10_4.py` v1.0 (~340 LOC): artefakt_ref-Prefix-Map + quellentyp-Enum-Map mit rede-Heuristik + voraussetzung-Normalisierung + sequenz_kontext-Backfill + primary_scpl_zone-Drop + _meta-Required-Default-Backfill + Unknown-_meta-Drop + --quellentext-only-Modus + --dry-run + --batch-data. Batch-Test auf N-K-data.json: 23 Total, 5 quellentext-migriert (alle 5/5 PASS gegen v3.10.4-Default-Schema), 18 non-quellentext geskippt (Legacy-Route per Fallback-Tabelle), 0 Errors. rede-Heuristik triggert propaganda-Klassifikation bei Bismarck "Blut und Eisen" + Buelow "Weltpolitik". inhalt + quelle byte-identisch. N-K-quellentext-Baseline v3.10.4-konform fuer C4-Regression. Vorheriger Eintrag **T-Pin-Hash + T-C3-Smoke DONE** — Gen-Repo main `07b58dc` → `f51d8bc` (Validator-PIN v3.10.3→v3.10.4 + PROVENANCE v1.2) → `a2141b9` (compute_sequenzkontext-Tool-Fix + BEFUND_T_C3_SMOKE v1.0 + Roadmap v1.2). T-C3-Smoke auf echtem N-K-Legacy-Material (mat-3-1 Bismarck) fand 4 Findings: Tool-Bug voraussetzung-Form GEFIXED + Legacy-Sequenzkontext-Luecke (erwartet) + Legacy-Schema-v3.10.4-FAIL (per Fallback-Tabelle erwartet, Migration-Tool-Scope) + M16/M17-Regex-schema-unabhaengig-robust. **Neuer Sub-Track C3.5 Legacy-Migration-Tool (2-3h) empfohlen VOR C4-bildquelle**, weil RA4-MV2-Hallu-Baseline N-K ist und Baseline v3.10.4-konform sein muss. Vorheriger Eintrag **Track C3 FF-merged Gen-Repo main `07b58dc`** — 5 C3-Commits (Schema v3.10.4 + 3 Tools + AGENT_MATERIAL.md + Specs v3.12.1/v0.1.2 + BEFUND_C3) + zusaetzlicher Commit TEST_AUDIT_ROADMAP v1.0. Feature-Branch geloescht. **TEST_AUDIT_ROADMAP v1.0 als SSOT fuer empirische Validation etabliert** (`docs/projekt/TEST_AUDIT_ROADMAP.md`): 6 Test-Ebenen + 8 Audit-Typen + kritischer Pfad ~4-6 Wochen bis Pilot-Freigabe + RA-Kanon (RA1 Pipeline, RA2 Didaktik, RA3 Engine, RA4 Medien, RA5 PM) analog Testrun-N-K-Audit. Next: T-Pin-Hash (30 min) → T-C3-Smoke (1-2h) → C4-bildquelle (Prio wegen MV2-Hallu-P0). Vorheriger Eintrag: **Track C3 DONE — Feature-Branch pushed, Merge PENDING User-Review BEFUND_C3**. 4 Gen-Repo-Commits auf `c3/dispatcher-integration-g3-phase-2-0b`: Schema v3.10.4 (`b5c6578`) + 3 Tools (`127cf22`) + AGENT_MATERIAL.md §2.0b/§Gate-Chain/§Verzeichnis-Cut-over (`cdc29a8`) + Specs v3.12.1+v0.1.2 (`6bfdc22`). E2E-Volllauf T3.3 PASS: Phase 2.0b deterministic, G1-M17 initial PASS, G3 FAIL auf Paul-Defekte (SQ-2 Waterberg + Q10 Adressat), Revisor → Re-G3 WARN (0 FAIL). 154k Tokens / 206s / 3 Opus-Dispatches / 1 Revisor-Iteration innerhalb Budget. **Bonus:** v0.1.2-Priorisierung eliminiert Severity-Emergenz-Klasse aus BEFUND_C2 §7.2 — Re-G3 Q10=PASS statt C2-WARN, WARN-Count halbiert (2 statt 4). Alle 3 C3-Akzeptanzkriterien PASS. 5 Offene Punkte nicht-blockend (Pin-Hash-Transition, Parallel-Skalierung, Glue-Code-Impl, Session-Path, G3-Titel-Uebergabe). C4-C9 Typ-Specializations entblockt nach Merge. Gen-Repo main unveraendert `1c86806`. Vorheriger Eintrag: **Track C2 MVP DONE + FF-merged** — Gen-Repo main HEAD `1c86806` (FF-merge von Feature-Branch `c2/revisor-modus-quellentext` mit 3 Commits: `c696196` reviewer v0.1.1 + `7e9c3b8` SUB_MATERIAL_QUELLENTEXT v3.12.0 + `1c86806` BEFUND_C2 + Spike-Plan v1.5). Feature-Branch lokal + remote geloescht. **Track C3 Dispatcher-Integration (G3-Phase + Phase 2.0b Sequenzkontext-Pre-Computation + Pro-Material-Verzeichnis-Struktur + Schema v3.10.4) entblockt und startet als naechster Track.** Aufwand laut Spike-Plan §C3: 3-5 Tage. Kritischer Pfad fuer Track-C-Abschluss. Offene Spec-Refinements aus C2 (§J.6 material_id + Reviewer v0.1.2 Recommendations-Priorisierung) opportunistisch in C3-Commits gebuendelt. Vorheriger Eintrag: **Track C2 Revisor-Modus DONE** — Feature-Branch `c2/revisor-modus-quellentext` Gen-Repo mit 3 Commits pushed, **KEIN Merge zu main vor User-Review BEFUND_C2**. Spec-Updates: reviewer-material-quellentext v0.1.0→v0.1.1 (BEFUND_C1 §6 Klarstellungen, Commit `c696196`) + SUB_MATERIAL_QUELLENTEXT v3.11.0→v3.12.0 (§0 Pre-Flight + §J Revisor-Modus 8 Sub-Abschnitte, Commit `7e9c3b8`) + BEFUND_C2 v1.0 + Spike-Plan v1.4→v1.5. End-to-End-Test (3 Opus-Dispatches, 202k Tokens, 395s): Generate→FAIL (SQ-2 Waterberg + Q10 Herero-Adressat) → Revise → Re-Review WARN (beide urspruengliche FAILs PASS, 4 WARN persistiert/emergiert). T2.2 Diff-Lokalisierung 100% (0 kollaterale Aenderungen, 5 inhalt-Edits alle Findings-adressierend, 5 _meta-Edits alle J.3-Ausnahmeliste-legitimiert oder Findings-adressierend). Beide Akzeptanzkriterien C2 erfuellt. Sub-Status MIXED bei Severity-Emergenz: Revisor-Korrektur "vernichten" triggert neuen Q10-WARN (didaktische Vorweginterpretation — Verb nicht im Zitat-Wortlaut). Reviewer-v0.1.2-Refinement-Kandidat (Recommendations-Priorisierung) offen, C3-gekoppelt. Offene Spec-Refinements: §J.6 `material_id` + Schema v3.10.4 fuer `review_iteration`/`review_warnings`/`revisor_notes` (C3-Dispatcher-Scope). Gen-Repo main unveraendert `16b2e21` — Merge pending. Vorheriger Eintrag **Track C1 MVP DONE** — A/B-Test `reviewer-material-quellentext` Opus vs. Sonnet via Cowork-Task-Tool-Pattern, beide 5/5 hart bestanden auf empirischen Smoke-Defekten (Waterberg SQ-1 + Herero-Adressat Q10), confidence 0.92 / 0.91, JSON parsbar. Severity-Kalibrierung divergent (Opus strenger semantisch-didaktisch, Sonnet strenger schema-formal), keine FAIL-Klassen-Inversion, False-Positive-Rate 0/0. Model-Default Opus beibehalten (n=1 Laufzeit-Anomalie Sonnet 3.5x langsamer — Default-Wechsel erfordert n>=3 Retest, opportunistisch in C2/C10). BEFUND `docs/projekt/F0e_REVIEW_AGENT_BEFUND_C1.md` v1.0 committed + FF-Merge Feature-Branch → main (Gen-Repo `bc41627..16b2e21`), Feature-Branch lokal + remote geloescht. Gen-Repo main HEAD `16b2e21`. Spike-Plan v1.3 → v1.4 mit §16 Nachtrag. Drei Spec-v0.1.1-Klarstellungen (MQ-HARD-STOP-Scope, Voelkermord-Severity-Rulebook, Prompt-Strictness-Sentinel) an C2-Auftakt zugeordnet. C2 Revisor-Modus-Integration jetzt entblockt. Vorheriger Eintrag F0e-AEF **Iteration-3 PASS** — Overlay v1.1 + Schema v3.10.3 empirisch gegen Baseline I1+I2 validiert (n=4, `mat-4-3`). §19.7-Akzeptanzkriterien vollstaendig erfuellt: Schema-Pass-Rate **4/4 = 100 %** (Baseline 3/4), D6-Inzidenz **0/4** (Baseline 1/4), Wortanzahl-Cap ≤ 180 **4/4** (108/108/109/118, Varianz Max/Min **1.09** vs. Baseline 2.74), Didaktik-Mittel **4.55** (Baseline 4.15, Min **4.2** vs. Baseline 3.8), D1-D5-Compliance 4/4, Patch-Zyklen 0. **P1-Cluster CLOSED** — PI 3.1 (D6+Schema-Strict), 3.2 (Content-Length), 3.6a (Inhalt-Prosa-Only), 3.7 (Quelle-SSOT) alle empirisch verifiziert. Commit **692e051** auf main gepusht, 36 Dateien. Deliverables: `runs/iteration-3/{PLAN,RUN_META,BEFUND_I3}.md` v1.0 + 4 Runs komplett (subagent_response + partial + merged + 2× gate-report + review), Dispatch-Prompt v2 SHA `640fb89f…`, Schema v3.10.3 Full SHA `f08df7ee…` + Partial SHA `0f3fe48e…` + PROVENANCE-Eintrag, Validator-PIN auf `f08df7ee…`. Promotion Schema v3.10.3 + Overlay v1.1 in Generator-Repo-Kern = separater Track, nicht F0e-Scope. Phase 19.B (PI 3.5/3.6b/3.8) + Phase 19.C (PI 3.4/3.9/3.10) offen. Vorheriger MIXED-Eintrag 2026-04-21 **SUPERSEDED** durch I3-PASS.)
**Modus:** F0e-AEF I3 PASS → Promotion-Track B DONE + gemerged → Pipeline-Aktivierungs-Smoke PASS → Track C0 PM-Verankerung DONE inkl. §22.13-22.17 Nachtraege → Track C1+C2+C3 + T-Pin-Hash + T-C3-Smoke + C3.5 + C-ARCHI-AUDIT S0-S7 DONE + User-Review-Gate Q1-Q9 DONE → **Track P.1 Block A+C DONE (Tag 1-3 zusammengezogen, ~5 Min Wall-Clock fuer 24 Frontmatter-Adds + Manifest-Bump v0.2.0). Next: Block B.1+B.2 F0B-Priming-Skill + escape-game-schema-Skill (Tag 4-5, 1.5 PT, manuelle Iteration).** Pfad H1→H2-Staffel bestaetigt. Q4 PI-Zukunft Option C+ (JSON-authoritativ + PI-Mirror + Sync-Hook). Q7 Code-Mode = Prod offiziell verankert. C4-C9 subsumiert in Track P.2. Total-Schaetzung bis Pilot-Freigabe: ~10-12 Wochen. Pilot-Freigabe weiter hinten im Pfad, erfordert T-Full-Game + 5-RA-Multi-RA-Audit. Tracks C3-C10 im Plugin-Format mit Hybrid-Dev-Workflow. Spec-Refinements aus C2 (§J.6 material_id + Reviewer v0.1.2 Recommendations-Priorisierung) opportunistisch in C3 gebuendelt. Track D reduziert zu Plugin-Publikation + E2E-Performance-Validierung + Checkpoint/Resume post-C10 (5-8 Tage). Phase 19.B/19.C weiter offen. Parallel: F0f Feld-Evidenz (#47) bleibt aktiv. F0g (#48) teil-entblockt durch P1-Cluster-Closure. F0b.2b bleibt DONE.
...
**Letzter Arbeitsschritt:** Session 2026-04-26 (Plugin v0.4.0 Dependency-Architektur, Variante C ~95 Min): Vollstaendige 4-Schicht-MCP-Manifest-Architektur als Antwort auf Phase-0.2-MCP-Gap. 8-Schritte-Cycle: (1) MCP-Recherche bestaetigt nur Wikipedia hat soliden Community-MCP, alle deutschen Bildungs-Quellen sind WebFetch. (2) plugin.json `dependencies`-Sektion mit requiredMcpServers/optionalMcpServers/useCaseMcpServers/webFetchFallbackSources/requiredCliTools/requiredPythonPackages. (3) PLUGIN_DEPENDENCIES.md (~250 Z) als Setup-Doku. (4) tools/setup-mcp.sh + tools/check_mcp_availability.py mit self-test PASS. (5) phase-prerequisites-check Skill Schritt 4b mit Phase-MCP-Pflicht-Matrix + Override-Mechanismus. (6) AGENT_INHALT Daten-Quellen-Strategie + E-D3-DATA-DEGRADED-Eskalation (KEIN silent-graceful-degrade mehr). (7) code-mode-launch.sh v0.4.0 mit 5-Stage-Pfad-Resolution (Args/ENV/Config/Glob/Default) + CC-Version-Check + MCP-Pre-Flight-Integration. (8) ~/.escape-game.env als User-Setup-Persistenz angelegt. **6 Findings CLOSED:** F-PB-15 HIGH (Wrapper-Pfad), F-PB-16 MED (CC-Version), F-PB-28 HIGH (MCP-Pre-Flight), F-PB-30 LOW (E-D-Granularitaet), F-PB-32 HIGH-with-Fallback (Wikimedia-WebFetch), F-PB-07 MED (Plugin-Install-Sichtbarkeit). Plugin-Version 0.3.3 → 0.4.0 (Minor-Bump). **Aggregat post-v0.4.0: 22 Findings, nur 2 HIGH offen** (F-PB-24 Hook-Verifikation pending, F-PB-32 closed-with-fallback). Pilot-Re-Run-Bereitschaft.
**Naechster Schritt:** Pilot-Re-Run-Sequenz fuer User: (1) CC `/exit`. (2) `claude plugin update escape-game-generator` (zieht v0.4.0). (3) `pipx install wikipedia-mcp` + `claude mcp add --transport stdio wikipedia -- wikipedia-mcp`. (4) `./tools/code-mode-launch.sh` (~/.escape-game.env-Persistenz aktiv, keine expliziten Args mehr). (5) Im neuen CC: `/escape-game-generator:resume-state` ODER frischer `/generate-game weimarer-republik-anfangsphase 7c 4`. **Vergleichs-Empirie:** Pre-MCP-Pilot (Phase 0.1+0.2 LLM-Memory) vs. Post-MCP-Pilot (Phase 0.2 mit Wikipedia-MCP). Erwartung: bessere Inhalts-Validitaet, MCP-Tool-Calls im Hook-Log sichtbar (F-PB-24-Verifikation indirekt via Bash-Tool-Calls), Subagent-data_source-Annotation aktiv.
