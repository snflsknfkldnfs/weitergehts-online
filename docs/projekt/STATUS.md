...
17. NEU bpb-Discovery-Mechanismus (R0.7 §14, dreistufig hybrid): **Stage 1** = statische Registry `escape-game-generator/data/bpb_dossier_registry.json` mit PM-gepflegten bpb-Dossier-Eintraegen (id, titel, url_root, thema_tags, lehrplan_bezug_gpg, unterartikel_anzahl, qualitaet_pm_note, zuletzt_pm_geprueft). **Stage 2** = Sub-Agent `bpb_discovery_agent` prueft erst Registry per thema_tag/lehrplan_bezug-Lookup, bei Miss Fallback auf WebSearch `site:bpb.de [thema] dossier` mit Scoring (URL-Muster, Titelmatch, Unterartikel-Hinweis), Top 3-5 Kandidaten an PM. **Stage 3** = Pflicht-Gate User-Bestaetigung, persistiert in `docs/agents/artefakte/[game-id]/bpb_discovery_bestaetigung.json` (kein Auto-Ingest). **Stage 4** = `bpb_primaerquellen_extraktor` laeuft nur auf bestaetigten URLs. Registry-Initial-Befuellung als separater PM-Task in Runde 2 Arbeitspaket 2b (getrennt von Sub-Agent-Implementation). Begruendung: reine User-Abfrage erzwingt Recherche-Pflicht (unzumutbar), reine WebSearch-Automation produziert False-Positives und verschwendet Token. Registry-First kombiniert beides.
**Offene Folgetickets:**
