...
**KEINE Quellenangabe im `inhalt`-Feld.** Quellenangaben (Autor, Titel, Jahr, Fundstelle, Aufbereitungsvermerk) gehoeren ausschliesslich in das `quelle`-Feld. KEIN `<cite>`-Tag im `inhalt`. Die Engine rendert `quelle` separat im ausblendbaren Quellenbereich. Dopplung fuehrt zu Q-Gate FAIL (Q6b).
