...
GAME_ID: weimarer-republik-anfangsphase
LETZTE_PHASE: Uebergangstabelle Zeile 2 → Phase 0.2 AGENT_INHALT FAIL-pending-0.2.M (2026-04-26). Output: docs/agents/artefakte/weimarer-republik-anfangsphase/INHALTSBASIS_weimarer-republik-anfangsphase.md. Recherche-Modus: LLM-Wissensbasis (kein MCP wikipedia in dieser Runtime; Transparenz-Notiz im Header). 26 Vertiefungsartikel + 17 Wikimedia-Kandidaten + 4 Primaerquellen game-weit. trigger_categories=[Konflikt, Macht-Asymmetrie, Revolution, Gewalt, Trauma, Demokratie-Gefaehrdung]. Q-Gates: QI1/QI3/QI-TD PASS; QI2/QI4/QI6/QI-RC1 PASS HIGH; QI5/QI-RC2/QI-RC3 PASS MEDIUM. **Zwei BLOCKER offen:** (a) QI-MV verschoben auf 0.2.M-Sub-Phase (agent-medienrecherche); (b) QI-TV M3-Titel "Versailles — 'Der Schandfrieden'" verletzt R-TITEL-3 (Perspektiv-Neutralitaet — "Schandfrieden" nimmt rechtsgerichteten Diskurs vorweg). Korrektur-Vorschlag C: "Versailles — Wieso sind alle so wuetend?" (Reportage-Headline aus DIDAKTIK_RAHMEN; neutralisierend-fragend).
NAECHSTE_AKTION: **STOP — USER-ENTSCHEIDUNG M3-Titel.** Nach PASS: (1) DIDAKTIK_RAHMEN + INHALTSBASIS + Mappen-Tabelle in PI auf neuen M3-Titel patchen; (2) Phase 0.2.M agent-medienrecherche dispatchen (Wikimedia-Dual-Kanal-Verifikation 17 Kandidaten); (3) bei 0.2.M-PASS → Phase-0.2-Gesamt-PASS → Phase 0.3 AGENT_SKRIPT + AGENT_ARTEFAKT.
MAPPEN_TOTAL: 4
MAPPEN_ABGESCHLOSSEN: 0
LETZTE_DEPLOY_CHECK_STATUS: NONE
LETZTE_DEPLOY_CHECK_TS: null
LETZTE_DEPLOY_CHECK_SCOPE: null
```

> **SPEICHERSTAND:** Der vorherige Game-Zustand (deutscher-nationalismus-kolonialismus, ABGESCHLOSSEN 2026-04-20) ist gesichert in:
> `weitergehts-online/docs/agents/artefakte/deutscher-nationalismus-kolonialismus/PROJECT_INSTRUCTIONS_SNAPSHOT.md`
...
## ═══ TRIPLE-ROOT-ARCHITEKTUR (ab v3.0, Track P.1 Block H 2026-04-25) ═══

Dieses Produktions-Setup arbeitet mit DREI Repositories als Context-Folder (vorher DUAL-ROOT, ab v3.0 erweitert um UNTERRICHTSEINWICKLUNG-Read-Only-Root):

```
GENERATOR              = escape-game-generator/      ← DIESES Repo (Plugin-Host, Agenten/Vertraege/Skills/Commands/Hooks LESEN + tools AUSFUEHREN)
TARGET                 = weitergehts-online/          ← Ziel-Repo (Artefakte SCHREIBEN, Engine LESEN, PM-Pflege STATUS+CHANGELOG)
UNTERRICHTSEINWICKLUNG = ~/weitergehts.online/Unterrichtseinwicklung/  ← Read-Only-Discovery (Lehrplan + Didaktik, Host-lokal Nicht-Git)
```

**Vollstaendige Triple-Root-Doku:** `docs/projekt/PLUGIN_ARCHITEKTUR_TRIPLE_ROOT.md` (Block H, Track P.1).
...
Daraus die drei Repos identifizieren:
- `GENERATOR_PATH` = /sessions/[session-name]/mnt/escape-game-generator
- `TARGET_PATH` = /sessions/[session-name]/mnt/weitergehts-online
- `UNTERRICHTSEINWICKLUNG_PATH` = /sessions/[session-name]/mnt/Unterrichtseinwicklung (Read-Only, ggf. nicht in jeder Session gemountet)

Ab diesem Punkt: ALLE Dateioperationen mit ABSOLUTEN Pfaden:
- Generator-Dateien: `{GENERATOR_PATH}/agents/ORCHESTRATOR.md`
- Target-Dateien: `{TARGET_PATH}/docs/agents/artefakte/[game-id]/`
- Lehrplan/Didaktik-Discovery: `{UNTERRICHTSEINWICKLUNG_PATH}/7c/Input_Artefake/Lehrplan/GPG_R7/`

**Pfad-Konvention:**
- Pfade mit `agents/`, `architektur/`, `checklisten/`, `tools/`, `skills/`, `commands/`, `hooks/` → GENERATOR-Repo
- Pfade mit `docs/agents/artefakte/`, `escape-games/`, `assets/` → TARGET-Repo
- Pfade mit `Unterrichtseinwicklung/` → UNTERRICHTSEINWICKLUNG-Root (READ-ONLY, niemals SCHREIBEN)

---

## ═══ RUNTIME-KONVENTION: STRICT-SEPARATION Cowork = Dev / Code-Mode = Prod (ab v3.0, verschaerft 2026-04-25) ═══

**STRICT-SEPARATION-PFLICHT (post-T-P.1-Smoke):**
- **Cowork = AUSSCHLIESSLICH Plugin-Development.** Plugin-Source-Files via Read/Edit-Tools editiert. Plugin selbst NIEMALS in Cowork installiert/geladen.
- **Code-Mode = AUSSCHLIESSLICH Plugin-Production (self-sustained).** Plugin via `claude plugin install` geladen. Game-Generierung autonom ohne Cowork-Abhaengigkeiten.

| Runtime | Rolle | Use-Cases | Plugin-geladen? |
|---|---|---|---|
| **Cowork** (Desktop-App) | **Dev-Umgebung** | Spec-/Skill-/Hook-Edits, Plugin-Iteration, Q-Gate-Tuning, PM-Arbeit, Audits, Reviews | **NEIN — Anti-Pattern** |
| **Code-Mode** (CLI-Tool) | **Prod-Umgebung** | Plugin-produktive Game-Generierung, Pilot-Einsatz, T-Full-Game-E2E, Skill-/Hook-/Command-Empirie | **JA via `claude plugin install`** |

**Anti-Pattern-Begruendung:** Plugin-Install in Cowork triggert Skills/Hooks waehrend Plugin-Source-Editing → kontraproduktiv (z.B. f0b-priming-Skill triggert bei Material-Spec-Edits, D.1-Hook blockt Plugin-internal-Test-Writes).

**Konsequenzen:**
- **Self-Sustained-Plugin in Code-Mode:** State-Machine (Q4 Option-C+) operiert autark, `/resume-state` aus Code-Mode-Session, Subagent-Dispatch via Code-Mode-Task-Tool.
- **Phase-3-Vereinheitlichung in Code-Mode:** Phase 0-3 laufen vollstaendig in Code-Mode. Cowork->Code-Mode-Wechsel entfaellt im Prod-Pfad.
- **Session-Split-Aufloesung:** Phase-2.1c-Session-Split-Pflicht ist Cowork-Constraint, durch Code-Mode-Checkpoint/Resume aufloesbar (Track-D-Feature post-P.2).
- **T-Full-Game in Code-Mode:** Empirisches Phase-2-Gate, kein Cowork-Hand-off.
- **Pilot-Einsatz:** Paul nutzt Plugin in Code-Mode fuer reale Game-Generierung.

**Empirie-Verifikations-Pfad:**
1. `claude plugin validate <root>` (statisch) — aus Cowork-Shell oder Code-Mode OK.
2. `claude plugin install` + `plugin list` — **Code-Mode-only** (Anti-Pattern in Cowork).
3. Skill-Auto-Trigger / Hook-Aktivierung / Command-Aufruf empirisch — **Code-Mode-only**.
4. E2E `/generate-game` (T-Full-Game) — **Code-Mode-only**.

**Vollstaendige Doku:** `docs/projekt/PLUGIN_ARCHITEKTUR_TRIPLE_ROOT.md` §3.
...
|---|---|---|
| `thema` | User | Weimarer Republik — Anfangsphase |
| `jahrgangsstufe` | User | R7 Mittelschule Bayern (Klasse 7c) |
| `schwierigkeit` | User (Standard: Basis) | Basis |
| `game_id` | Abgeleitet aus thema | weimarer-republik-anfangsphase |
| `lehrplanbezug` | Auto: AGENT_DIDAKTIK (Phase 0.1) | Primaer LB3 Politik+Gesellschaft (K_04); Sekundaer LB2 Zeit+Wandel (K_01, K_02; NEBEN K_08) |
| `mappen_anzahl` | User-Override (statt Auto: AGENT_DIDAKTIK) | 4 |
...
GENERATOR-REPO:
  ...
  [ ] {G}/agents/agent-material-design.md
  [ ] {G}/agents/agent-material-dispatcher.md
  [ ] {G}/agents/AGENT_MATERIAL.md (Legacy-Anker, DEPRECATED post-B.1 2026-04-25)
...
| 7 | 0.4 | 1.1 agent-material-design (Design-Modus) → MATERIAL_GERUEST | VERTRAG_PHASE_2-1_MATERIAL.md + agent-material-design.md (ehem. AGENT_MATERIAL.md) | Cowork | Materialtyp-Zuordnung, TB-Abdeckungs-Nachweis |
...
| 15 | SPLIT → 2.2a | Progressionsplan | VERTRAG_PHASE_2-2a_PROGRESSIONSPLAN.md + agent-raetsel-progressionsplan.md (ehem. AGENT_RAETSEL) | Cowork | Liest `materialien/*.json` **nur Metadaten**, NICHT Volltext |
...
| 17 | 2.2b | 2.2c Aufgaben-Cross-Konsistenz | VERTRAG_PHASE_2-2c_CROSS.md + agent-raetsel-dispatcher.md (ehem. AGENT_RAETSEL) | Cowork | Q-Gate A5, A8-A10, A12 |
...
2. **Ruecklauf-Zuordnung:** Jeder Mangel wird dem zustaendigen Agent zugeordnet:
   ...
   | Material-Qualitaet / MQ1-MQ5 / typ-spezifisch | SUB_MATERIAL_* (via agent-material-dispatcher) | 2.1 |
   | Didaktik-Review-Achsen D1-D4 | agent-material-dispatcher + relevante SUB_MATERIAL_* | 2.1b |
   | Raetsel-Design / A1-A12, A25, A26 | agent-raetsel-progressionsplan + agent-raetsel-dispatcher (ehem. AGENT_RAETSEL) + SUB_AUFGABE_* | 2.2a-c |
   | Cross-Konsistenz (Material oder Aufgaben) | agent-material-dispatcher bzw. agent-raetsel-dispatcher (ehem. AGENT_RAETSEL) | 2.1c / 2.2c |
