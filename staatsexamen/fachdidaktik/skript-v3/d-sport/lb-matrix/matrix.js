...
(function () {
  ...
  function renderPilotUE(ue, phasenSchema) {
    ...
    // SPORT-SPEZIFISCH: Hallenplan + Helferkonzept + Sicherheit als Box
    if (ue.hallenplan || ue.helferkonzept || ue.sicherheit) {
      const sportBox = h('div', { class: 'mx-sport-buv' });
      sportBox.appendChild(h('div', { class: 'mx-sport-buv__h' }, 'Sport-BUV · Hallenplan + Sicherheit'));
      if (ue.hallenplan)    sportBox.appendChild(h('div', { class: 'mx-sport-buv__row' }, h('b', null, 'Hallenplan'), ue.hallenplan));
      if (ue.helferkonzept) sportBox.appendChild(h('div', { class: 'mx-sport-buv__row' }, h('b', null, 'Helferkonzept'), ue.helferkonzept));
      if (ue.sicherheit)    sportBox.appendChild(h('div', { class: 'mx-sport-buv__row' }, h('b', null, 'Sicherheit'), ue.sicherheit));
      el.appendChild(sportBox);
    }

    // SPORT-SPEZIFISCH: Sachanalyse (BUV-Pflicht)
    if (ue.sachanalyse) {
      const sa = ue.sachanalyse;
      const saBox = h('div', { class: 'mx-sport-sachanalyse' });
      saBox.appendChild(h('div', { class: 'mx-sport-sachanalyse__h' }, 'Sachanalyse (BUV)'));
      if (sa.bewegungsphasen)     saBox.appendChild(h('div', { class: 'mx-sport-sachanalyse__row' }, h('b', null, 'Bewegungsphasen'), sa.bewegungsphasen));
      if (sa.biomechanik)         saBox.appendChild(h('div', { class: 'mx-sport-sachanalyse__row' }, h('b', null, 'Biomechanik'), sa.biomechanik));
      if (sa.fehlerbilder)        saBox.appendChild(h('div', { class: 'mx-sport-sachanalyse__row' }, h('b', null, 'Fehlerbilder + Korrektur'), sa.fehlerbilder));
      if (sa.lernvoraussetzungen) saBox.appendChild(h('div', { class: 'mx-sport-sachanalyse__row' }, h('b', null, 'Lernvoraussetzungen'), sa.lernvoraussetzungen));
      el.appendChild(saBox);
    }

    // Didaktische Reduktion (Sport-spezifisch + BUV-Pflicht)
    if (ue.didaktische_reduktion) {
      const dr = h('div', { class: 'mx-sport-redu' });
      dr.appendChild(h('span', { class: 'mx-sport-redu__h' }, 'Didaktische Reduktion'));
      dr.appendChild(h('span', null, ue.didaktische_reduktion));
      el.appendChild(dr);
    }
  ...
  function renderSlideoverContent(cellKey, cell) {
    ...
    // Cell-Titel: LB/Sport-Handlungsfeld als prominenter Header
    frag.appendChild(h('h2', { class: 'mx-slideover__title', id: 'mx-slideover-title' },
      `${cell.gb_titel || cell.lb_titel || cellKey}`));
    frag.appendChild(h('div', { class: 'mx-slideover__sub' },
      `${jgstObj?.label || jgst} · ${cell.gb || cell.lb || ''} · ${cell.ke_anzahl} Kompetenzerwartung${cell.ke_anzahl > 1 ? 'en' : 'en'}`));
  ...
  function renderGrid() {
    const grid = h('div', { class: 'mx-grid', role: 'grid', 'aria-label': 'LB-Matrix Sport' });
