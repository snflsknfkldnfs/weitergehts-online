...
var EscapeEngine = (function () {
  ...
  // Beschriftung der Typ-Badge im Aufgaben-Header (T2: aus rd-inject.js geholt).
  var TYP_LABEL = {
  ...
  function init(mappeId) {
    ...
    _loadData()
      .then(function (data) {
        ...
        // Akten-Label im Header (datengetrieben; ergaenzt nur, was fehlt)
        _renderAktenLabel();
  ...
  function showTipp(mappeId, aufgabeIndex, stufe) {
    ...
    for (var i = 0; i < tipps.length; i++) {
      if (tipps[i].stufe === stufe) {
        ...
        // Bug #2: sprachbewusster Tipp — DaZ bekommt die uebersetzte Fassung (DE bleibt Anker).
        // Sprache scope-unabhaengig aus dem DOM lesen (getCurrentSprache lebt im Sprach-
        // Extension-IIFE, showTipp im Core-IIFE — setSprache spiegelt die Sprache ins dataset).
        var _tl = (document.documentElement && document.documentElement.dataset.sprache) || 'de';
        var _tu = tipps[i].text_uebersetzung;
        if (_tl !== 'de' && _tu && _tu[_tl]) return _tu[_tl];
   ...
   * Prueft jede Mappe auf vorab_freigeschaltet === true und markiert
  ...
  var _DOK_FORMS = ['formular', 'brief', 'erlass_bekanntmachung', 'zeitungsartikel',
                    'plakat_flugblatt', 'rede_transkript', 'tagebuchseite', 'quelle_generisch'];

  /** @private Dokument-Form-Chrome (Design-Increment #4): Klassen + Kopf/Schluss/Kennzeichnung.
      container: das Material-Element; inhaltDiv: der bereits befuellte inhalt-Container. */
  function _applyDokumentForm(container, inhaltDiv, mat) {
    var dok = mat.dokument;
    if (!dok || !dok.form) return;
    var form = _DOK_FORMS.indexOf(String(dok.form)) === -1 ? 'quelle_generisch' : String(dok.form);
    container.className += ' material--dok material--dok-' + form;
    inhaltDiv.className += ' dok__korpus';
    if (dok.kopfzeile) {
      var kopf = document.createElement('div');
      kopf.className = 'dok__kopf';
      kopf.textContent = dok.kopfzeile;
      container.insertBefore(kopf, inhaltDiv);
    }
    if (dok.schlusszeile) {
      var schluss = document.createElement('div');
      schluss.className = 'dok__schluss';
      schluss.textContent = dok.schlusszeile;
      _insertAfter(container, schluss, inhaltDiv);
    }
    if (dok.kennzeichnung) {
      var kz = document.createElement('p');
      kz.className = 'dok__kennzeichnung';
      kz.textContent = dok.kennzeichnung;
      var anchor = dok.schlusszeile ? inhaltDiv.nextSibling : inhaltDiv;
      _insertAfter(container, kz, anchor);
    }
  }

  /** @private */
  function _insertAfter(parent, node, ref) {
    if (ref && ref.nextSibling) { parent.insertBefore(node, ref.nextSibling); }
    else { parent.appendChild(node); }
  }

  /** @private */
  function _appendBegleitBild(container, mat) {
    var bb = mat.begleit_bild;
    if (!bb || !bb.src) return;
    var fig = document.createElement('figure');
    fig.className = 'material__begleitbild';
    var img = document.createElement('img');
    img.src = bb.src;
    img.alt = bb.motiv || '';
    img.loading = 'lazy';
    fig.appendChild(img);
    var capText = bb.motiv || '';
    // CC-Lizenzen verlangen Namensnennung am Pixel; public_domain braucht keine.
    if (bb.attribution && typeof bb.lizenz_klasse === 'string' && bb.lizenz_klasse.indexOf('cc_') === 0) {
      var lizenzLabel = { cc_by: 'CC BY', cc_by_sa: 'CC BY-SA' }[bb.lizenz_klasse] || bb.lizenz_klasse;
      capText = (capText ? capText + ' — ' : '') + 'Bild: ' + bb.attribution + ' (' + lizenzLabel + ')';
    }
    if (capText) {
  ...
  function _renderMaterialQuelle(mat) {
    ...
    _appendBegleitBild(blockquote, mat);

    var inhaltDiv = document.createElement('div');
    inhaltDiv.className = 'material__inhalt';
    inhaltDiv.innerHTML = mat.inhalt || '';
    blockquote.appendChild(inhaltDiv);
    _applyDokumentForm(blockquote, inhaltDiv, mat);
  ...
  function _renderMaterialBild(mat) {
    ...
    img.alt = mat.titel || '';
    img.loading = 'lazy';
    // Foto-Mount (T2: entsteht mit dem Bild, frueher nachtraeglich umgehaengt)
  ...
  function _renderMaterialKarte(mat) {
    ...
    if (inhalt.indexOf('<svg') !== -1) {
      ...
      img.alt = mat.titel || '';
      img.loading = 'lazy';
      // Karten-Rahmen (T2: entsteht mit der Karte, frueher nachtraeglich umgehaengt)
  ...
  function _renderMaterialTagebuch(mat) {
    ...
    h3.textContent = mat.titel || '';
    article.appendChild(h3);
    _appendBegleitBild(article, mat);

    var inhaltDiv = document.createElement('div');
    inhaltDiv.className = 'material__inhalt tagebuch__text';
    inhaltDiv.innerHTML = mat.inhalt || '';
    article.appendChild(inhaltDiv);
    _applyDokumentForm(article, inhaltDiv, mat);
  ...
  function _renderMappeV1(mappe, progress) {
    ...
    // Leitfragen-Strip + Stempelfeld — hier stehen alle .aufgabe-Knoten
  ...
  function _renderAufgabe(aufgabe, index, geloest, total) {
    ...
    // Typ-Badge — entsteht mit der Aufgabe, nicht nachtraeglich (T2)
  ...
  function _updateFortschritt(mappe, progress) {
    ...
    // Stempelfeld mitziehen — jede _check<Typ>-Funktion endet hier.
    _syncStempelfeld();
  }

  // ========================================================================
  // Akten-Look (T2: aus vendor/rd-inject.js in die Engine geholt)
  //
  // Frueher setzte ein Nachruestsatz diese Elemente per MutationObserver nach
  // jedem Render obendrauf. Die Engine kennt ihren eigenen Renderzeitpunkt —
  // sie erzeugt sie jetzt selbst. Typ-Badge, Karten-Rahmen und Foto-Mount
  // entstehen direkt in den jeweiligen Render-Funktionen.
  // ========================================================================

  /**
   * Akten-Label im Mappen-Header. Der Builder schreibt es bereits statisch ins
   * HTML (aus mappen[i].akten_label bzw. der Mappen-Nummer); diese Funktion
   * ergaenzt es nur, wenn es fehlt — etwa in handgebauten Seiten.
   * @private
   */
  ...
  /**
   * Leitfragen-Strip mit Stempelfeld unter dem Header. Wird gerufen, wenn alle
   * .aufgabe-Knoten und der Einstieg im DOM stehen.
   * @private
   */
  ...
  /**
   * Haelt die Stempelfeld-Zellen am Solved-State der Aufgaben.
   * @private
   */
  ...
  // ========================================================================
  // Spiel-Uebersicht (index.html) — T3: aus dem Inline-Skript der index.html
  // in die Engine geholt. Die Startseite wurde sonst an zwei Stellen gepflegt,
  // und die Website ruft initUebersicht() direkt aus der Engine auf.
  // ========================================================================
  ...
  /**
   * Dossier-Zeile der Mappen-Kachel. Fach und Jahrgangsstufe stehen in meta —
   * frueher waren sie als CSS-content festgeschrieben und damit pro Spiel gleich.
   * @private
   */
  function _dossierLabel(meta) {
    if (meta && meta.dossier_label) return meta.dossier_label;
    var teile = [];
    if (meta && meta.fach) teile.push(meta.fach);
    if (meta && meta.jahrgangsstufe) teile.push(meta.jahrgangsstufe);
    return teile.length ? 'DOSSIER · ' + teile.join(' / ') : 'DOSSIER';
  }
  ...
  function _renderMappeKarte(mappe, index, progress, allProgress, meta) {
    ...
    dossier.textContent = _dossierLabel(meta);
  ...
  function _renderStartseite(data) {
    ...
    _setTextById('narrativ-text', meta.narrativ || 'Willkommen!');

    // Akten-Label als erstes Kind des Titel-Schilds — MUSS nach dem
    // textContent-Setzen laufen, sonst raeumt es das Label wieder weg.
    ...
    if (titelEl && !titelEl.querySelector('.akten-label')) {
      ...
      akte.textContent = meta.akten_label || 'LAGEBESPRECHUNG';
    ...
    for (var i = 0; i < mappen.length; i++) {
      var mp = (allProgress.mappen && allProgress.mappen[mappen[i].id]) || null;
      container.appendChild(_renderMappeKarte(mappen[i], i, mp, allProgress, meta));
    }
  }
  ...
  function initUebersicht() {
    _loadData()
      ...
      .catch(function (err) {
        ...
        if (hinweis) hinweis.textContent = 'Keine data.json gefunden.';
      });
  }
...
(function () {
  ...
  var SUPPORTED_LANGS = ['de', 'ru', 'uk', 'ar'];
  var LANG_LABEL = { de: 'DE', ru: 'RU', uk: 'UK', ar: 'AR' };
  var LANG_LABEL_LONG = { de: 'Deutsch', ru: 'Русский', uk: 'Українська', ar: 'العربية' };
  ...
  var I18N = {
    de: {
      ...
      pruefen_antwort: 'Antwort prüfen',
      pruefen_zuordnung: 'Zuordnung prüfen',
      pruefen_antworten: 'Antworten prüfen',
      pruefen_reihenfolge: 'Reihenfolge prüfen',
      bitte_waehlen: '-- Bitte wählen --',
      beruecksichtige: 'Berücksichtige:',
      antwort_placeholder: 'Schreibe deine Antwort hier...',
      tipp_label: 'Tipp'
    },
    ru: {
      ...
      pruefen_antwort: 'Проверить ответ',
      pruefen_zuordnung: 'Проверить соответствие',
      pruefen_antworten: 'Проверить ответы',
      pruefen_reihenfolge: 'Проверить порядок',
      bitte_waehlen: '-- Выберите --',
      beruecksichtige: 'Учти:',
      antwort_placeholder: 'Напиши свой ответ здесь...',
      tipp_label: 'Подсказка'
    },
    uk: {
      ki_box_titel: 'Отримати допомогу ШІ',
      ki_box_button: 'Скопіювати запит для ШІ',
      ki_box_kopiert: 'Скопійовано ✓',
      ki_box_hint: 'Встав запит у ШІ й постав своє запитання.',
      ki_toast_open: 'Вкладку відкрито — обери модель «Claude Haiku» і натисни Enter',
      ki_toast_blocked: 'Спливне вікно заблоковано — запит скопійовано, відкрий duck.ai вручну',
      sprache_button_label: 'Вибрати мову',
      tooltip_close: 'Закрити',
      nur_de_hinweis: '(лише DE)',
      pruefen_antwort: 'Перевірити відповідь',
      pruefen_zuordnung: 'Перевірити відповідність',
      pruefen_antworten: 'Перевірити відповіді',
      pruefen_reihenfolge: 'Перевірити порядок',
      bitte_waehlen: '-- Обери --',
      beruecksichtige: 'Врахуй:',
      antwort_placeholder: 'Напиши свою відповідь тут...',
      tipp_label: 'Підказка'
    },
    ar: {
      ...
      pruefen_antwort: 'تحقق من الإجابة',
      pruefen_zuordnung: 'تحقق من المطابقة',
      pruefen_antworten: 'تحقق من الإجابات',
      pruefen_reihenfolge: 'تحقق من الترتيب',
      bitte_waehlen: '-- اختر --',
      beruecksichtige: 'ضع في اعتبارك:',
      antwort_placeholder: 'اكتب إجابتك هنا...',
      tipp_label: 'تلميح'
    }
  };
  ...
  function _walkAndWrap(node, pattern, erklaerungObj, done) {
    ...
    // SVG-Teilbaeume sind tabu: ein HTML-<button> ohne <foreignObject> rendert im
    // SVG-Namespace nicht — der ersetzte Text-Node verschwaende ersatzlos am Pixel.
  ...
  // Bug #2: sichtbaren Aufgaben-Text sprachbewusst tauschen — DE-Original einmal in
  // dataset.deText sichern, dann uebersetzen bzw. auf DE zuruecksetzen. Die Loesungs-Anker
  // (input.value / data-value / data-wort / loesung) bleiben UNBERUEHRT (Pruefung bleibt DE).
  function _swapAufgText(el, translated, lang) {
    if (!el) return;
    ...
    if (lang !== 'de' && translated) {
      ...
      el.textContent = el.dataset.deText;
      el.dir = '';
    }
  }

  // Lueckentext-Gap-Text: die Text-Segmente ZWISCHEN den Luecken (Text-Knoten im
  // .aufgabe__lueckentext) durch die uebersetzten Segmente ersetzen; die Luecken-Elemente
  // (Inputs/Pool-Spans) bleiben in Position. DE aus text_mit_luecken (split '___'),
  // uebersetzt aus luecken_uebersetzung[lang].text_mit_luecken (split '{{N}}').
  function _translateLueckenGaps(sec, auf, lang) {
    var box = sec.querySelector('.aufgabe__lueckentext');
    if (!box) return;
    var deSegs = String(auf.text_mit_luecken || '').split('___');
    var segs = deSegs;
    if (lang !== 'de') {
      var lu = auf.luecken_uebersetzung && auf.luecken_uebersetzung[lang];
      var tml = lu && lu.text_mit_luecken;
      if (tml) {
        var cand = String(tml).split(/\{\{\s*\d+\s*\}\}/);
        if (cand.length === deSegs.length) segs = cand;  // nur bei gleicher Segmentzahl, sonst DE
      }
    }
    var ti = 0;
    for (var n = 0; n < box.childNodes.length; n++) {
      var node = box.childNodes[n];
      if (node.nodeType === 3) {                          // Text-Knoten = ein Segment
        if (ti < segs.length) node.nodeValue = segs[ti];
        ti++;
      }
    }
    box.dir = (lang === 'ar') ? 'rtl' : '';
  ...
  function _translateAufgabe(sec, auf, lang) {
    ...
    // -- Typ-spezifische Inhalte + statische Labels (Bug #2: vorher nur frage + MC-optionen) --
    var _t = I18N[lang] || I18N.de;
    var _de = (lang === 'de');
    if (auf.typ === 'zuordnung') {
      var _pu = (auf.paare_uebersetzung && auf.paare_uebersetzung[lang]) || null;
      var _begr = sec.querySelectorAll('.aufgabe__zuordnung-begriff');
      for (var _bi = 0; _bi < _begr.length; _bi++) {
        _swapAufgText(_begr[_bi], (!_de && _pu && _pu[_bi]) ? _pu[_bi].links : null, lang);
      }
      var _deR = auf.optionen || [];
      var _opts = sec.querySelectorAll('.aufgabe__zuordnung-select option');
      for (var _oi = 0; _oi < _opts.length; _oi++) {
        var _ov = _opts[_oi];
        if (_ov.value === '') { _swapAufgText(_ov, _de ? null : _t.bitte_waehlen, lang); continue; }
        var _ri = _deR.indexOf(_ov.value);            // DE-value = Anker -> Index -> Uebersetzung
        _swapAufgText(_ov, (!_de && _pu && _ri !== -1 && _pu[_ri]) ? _pu[_ri].rechts : null, lang);
      }
    } else if (auf.typ === 'reihenfolge') {
      var _eu = (auf.elemente_uebersetzung && auf.elemente_uebersetzung[lang]) || null;
      var _deE = auf.optionen || [];
      var _items = sec.querySelectorAll('.aufgabe__reihenfolge-item');
      for (var _ii = 0; _ii < _items.length; _ii++) {
        var _txtEl = _items[_ii].querySelector('.aufgabe__reihenfolge-text');
        var _ei = _deE.indexOf(_items[_ii].getAttribute('data-value'));  // data-value = Anker
        _swapAufgText(_txtEl, (!_de && _eu && _ei !== -1) ? _eu[_ei] : null, lang);
      }
    } else if (auf.typ === 'lueckentext') {
      var _lu = (auf.luecken_uebersetzung && auf.luecken_uebersetzung[lang]) || null;
      var _wpool = (_lu && Array.isArray(_lu.wortpool)) ? _lu.wortpool : null;
      var _deP = auf.antwortpool || [];
      var _pws = sec.querySelectorAll('.aufgabe__pool-wort');
      for (var _wi = 0; _wi < _pws.length; _wi++) {
        var _pidx = _deP.indexOf(_pws[_wi].getAttribute('data-wort'));   // data-wort = Anker
        _swapAufgText(_pws[_wi], (!_de && _wpool && _pidx !== -1) ? _wpool[_pidx] : null, lang);
      }
      _translateLueckenGaps(sec, auf, lang);
    } else if (auf.typ === 'freitext-code') {
      var _tfu = (auf._meta && auf._meta.teilfragen_uebersetzung
                  && auf._meta.teilfragen_uebersetzung[lang]) || null;
      var _lis = sec.querySelectorAll('.aufgabe__teilfragen-liste li');
      for (var _li = 0; _li < _lis.length; _li++) {
        _swapAufgText(_lis[_li], (!_de && _tfu && _tfu[_li]) ? _tfu[_li] : null, lang);
      }
      var _lbl = sec.querySelector('.aufgabe__teilfragen-label');
      if (_lbl) _swapAufgText(_lbl, _de ? null : _t.beruecksichtige, lang);
      var _ta = sec.querySelector('.aufgabe__textarea');
      if (_ta) {
        if (_ta.dataset.dePh === undefined) _ta.dataset.dePh = _ta.getAttribute('placeholder') || '';
        _ta.setAttribute('placeholder', _de ? _ta.dataset.dePh : (_t.antwort_placeholder || _ta.dataset.dePh));
      }
    }
    // Submit-Button (typ-spezifisch) + Tipp-Trigger-Label
    var _subMap = { 'multiple-choice': 'pruefen_antwort', 'zuordnung': 'pruefen_zuordnung',
                    'lueckentext': 'pruefen_antworten', 'reihenfolge': 'pruefen_reihenfolge',
                    'freitext-code': 'pruefen_antwort' };
    var _subEl = sec.querySelector('.aufgabe__submit');
    if (_subEl && _subMap[auf.typ]) _swapAufgText(_subEl, _de ? null : _t[_subMap[auf.typ]], lang);
    var _trigs = sec.querySelectorAll('.tipp__trigger');
    for (var _tri = 0; _tri < _trigs.length; _tri++) {
      var _trg = _trigs[_tri];
      if (_trg.dataset.deText === undefined) _trg.dataset.deText = _trg.textContent;
      var _num = (_trg.dataset.deText.match(/\d+/) || [''])[0];
      _trg.textContent = (_de ? 'Tipp' : (_t.tipp_label || 'Tipp')) + (_num ? ' ' + _num : '');
    }
